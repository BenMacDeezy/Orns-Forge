...
#
# SOURCE-SAFE (2026-07-23): the logic lives in forge_budget_guard(), which
# RETURNS rather than exits, so `task-dispatch.sh` can source this file and run
# it in the SAME bash process as the provenance flag — one spawn per dispatch
...
forge_budget_guard() {
  local input="$1"
  local cap key counter n msg
  cd "${CLAUDE_PROJECT_DIR:-.}" 2>/dev/null || return 0
  [ -f .forge/forge.md ] || return 0
...
if [ "${BASH_SOURCE[0]}" = "$0" ]; then
  forge_budget_guard "$(cat 2>/dev/null)"
  exit 0
fi
