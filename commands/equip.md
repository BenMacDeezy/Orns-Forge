...
- Present a ranked proposal via structured option cards — FIND / CREATE /
  WIRE / SKIP per gap, plus PORT wherever the inventory turned up a
  non-Forge agent file. Install, create, queue, port, or enable NOTHING
  without explicit approval on the cards.
