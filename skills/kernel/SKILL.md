...
- **Worktree probe + stale sweep.** Probe worktrees once
  (`references/degraded-worktree-mode.md`, NORMATIVE); on failure the session
  runs DEGRADED — sequential + in-place for Claude and codex, stated in the
  charter. Then `git worktree list`; any worktree from a Forge dispatch whose
  task has no active claim is a dead-session orphan — flag it in the session
  report for human cleanup. Never auto-delete a worktree.
...
- worktrees work here (SYNC probe); DEGRADED = sequential-only, no batch;
...
Record the eligibility decision (batch, or why sequential) in each task's
Routing record; anything ineligible runs sequential. Shard eligibility (GATE, not wave): `docs/conventions.md`, "Shard-eligibility predicate." NORMATIVE.

### Executor (Features: workflow-executor)

WHEN forge.md's Features set `workflow-executor: on` AND the harness offers
a tool named `Workflow` (check your tool list; if absent or unloadable,
never assume it exists), read
`skills/kernel/references/workflow-executor.md` before GATE→DISPATCH —
NORMATIVE: the parallel-batch and ship-review Workflow scripts, the two
carried-over hard rules, budget accounting, and crash-recovery/resume all
live there. Otherwise (toggle off or tool absent) skip to the sequential
loop below — behavior is identical, only the executor changes.
...
This table's Critical/forensic row (opus/max) is for a whole TASK routed
critical/forensic — not `forge-security`'s default; the ship protocol's
security pass (`forge:ship`) runs at its opus/high default regardless.

`opus` is the strongest tier the router assigns. `fable` is a
human-authorized escalation, never a route — `docs/conventions.md`,
"Model vocabulary — fable amendment (2026-07-17)", NORMATIVE.

Overrides allowed with one stated line; check forge.md Routing overrides
first, log the route in the Routing record. CONTEXT auto-includes tagged facts (conventions Mechanical-include rule).
...
sequential (one task at a time) otherwise. A sequential worker runs in the
BACKGROUND, awaited (never foreground-blocked), so the fallback watchdog
supervises it — `docs/conventions.md`, "Supervised sequential dispatch —
2026-07-24", NORMATIVE. Pass the routed model as the
...
cross-model review, worktree-OR-in-place mechanics (§7.2; degraded mode
runs codex in-place, `references/degraded-worktree-mode.md`),
workspace-write pairing, retry-then-force contract, equal-or-higher
verify floor. Any gate layer
unmet: routes to a Claude `forge-worker` exactly as if no `provider:`
field were present.

**Parallel dispatch.** WHEN GATE's parallel-eligibility test passed, read
`skills/kernel/references/parallel-dispatch.md`'s "ROUTE + DISPATCH —
Parallel dispatch" section first — NORMATIVE: claim-batch-first, worktree
isolation, Hard Rule 4, partial-failure/session-death handling. Ineligible
batches skip it. While work is in flight the kernel waits for completion
notifications, not each hook fire — `docs/conventions.md`, "Idle-wait
discipline — 2026-07", NORMATIVE. Shard expansion (fg-a10814): `skills/kernel/references/parallel-dispatch.md` — worktree/shard, 1 window, #N. NORMATIVE.
