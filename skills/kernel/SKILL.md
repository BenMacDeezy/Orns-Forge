...
**Provider-field routing (Phase 2, fg-c0111).** WHEN a task's dispatch
contract carries a `provider:` field and the active profile's `role-worker`
resolves to that provider, read
...
2. **Verifier spawn (standard/full tier, the default).** Spawn
   ...
   **Visual gate routing.** When the task's acceptance criteria are primarily
   ...
   At VERIFY, before spawning the mode-2 verifier, read
   references/verify-modes.md (NORMATIVE) when the diff looks docs/config-only
   and you are considering the reduced checklist; when in doubt, run full
   verification.
3. **Finder / kernel-synthesis (report tasks only).** At VERIFY
   mode-selection, read references/verify-modes.md (NORMATIVE) when a task's
   sole deliverable is a findings report, before choosing mode 3.
...
advisory: you may stop early on your spend estimate, but it is not the
