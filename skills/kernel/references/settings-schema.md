...
| `session-token-cap` | `"none"\|int` | none | "none" or a positive integer | Advisory only — the model may stop early on its own spend estimate; not the enforcement mechanism. | `human-set-cap` |
