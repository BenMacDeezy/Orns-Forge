...
| `max-parallel-tasks` | `"auto"\|"none"\|int` | auto | "auto", "none", or a positive integer | Sliding-window cap on SIMULTANEOUS worker spawns — never a cap on how many tasks a session eventually runs. `auto` derives it per machine as `min(cores - 2, 16)` floored at 1 (`validate_config.resolve_max_parallel_tasks()`); `none` is unbounded and the human owns resource contention. | — |
