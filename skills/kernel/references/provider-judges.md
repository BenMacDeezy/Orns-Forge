...
read-only judges only; Phase 2 worker dispatch (`provider:` field routing)
is a separate future task this file does not build.
...
### 7.1 Route gate — when a task's build dispatches to a provider

WHEN a task's dispatch contract carries a `provider:` field, THE SYSTEM
SHALL treat that field as authoritative for that task's BUILD dispatch
only when ALL of the following hold, checked at ROUTE:
...
Any one condition unmet: the `provider:` field is inert for this
dispatch and the task routes to a Claude `forge-worker` exactly as if no
`provider:` field were present — the same graceful-degrade shape
