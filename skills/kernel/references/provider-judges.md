...
SHALL run it inside the SAME isolation mode an in-harness Claude
`forge-worker` would get FOR THE CURRENT DISPATCH — the rule is
"identical to the Claude worker beside it," not "always a worktree." This
cuts two ways:
- **Parallel / worktree-capable dispatch** (a Claude worker here would get
  `isolation: "worktree"`): the provider worker gets the SAME worktree
  isolation — `skills/kernel/references/parallel-dispatch.md`'s worktree
  mechanics apply unchanged, cited, not restated.
- **Sequential / degraded dispatch** (a Claude worker here runs IN-PLACE
  against the main tree with no worktree — the sequential path
  parallel-dispatch.md says "needs none of this" machinery, and the whole
  session when the SYNC worktree probe reports DEGRADED): the codex worker
  runs IN-PLACE too, `--sandbox workspace-write` confining writes to the
  repo root exactly as it would confine them to a worktree. A codex
  mutating worker is NEVER excluded merely because worktrees are
  unavailable — full mechanics in `skills/kernel/references/
  degraded-worktree-mode.md` (NORMATIVE), including in-place skill
  materialization, the §8 `.forge-dispatch/` INTEGRATE exclusion becoming
  load-bearing, and in-place bounce-revert.

Hard Rule 4 holds identically in BOTH cases: the external worker never
touches `.forge/`; every `.forge/` write is kernel-only, on the main
branch, never inside the worker's tree. The worker never commits — the
kernel integrates the resulting diff per fg-a10815's shard
...
no new merge path is introduced for external workers, in-place or worktree.
