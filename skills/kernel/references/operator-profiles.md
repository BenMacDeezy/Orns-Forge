...
- **Per-role assignment keys** — `role-plan-refuter`, `role-spec-review`,
  ...
  warn-not-fail posture. **`role-worker` is R1-live (Phase 2 shipped
  2026-07-22, `bm-atomic-doc-fix-canonical-route`)**: spec-e8a3 shipped
  Phase 1 (`role-plan-refuter`, `role-spec-review`, `role-co-verifier` —
  read-only judges) first; the Phase-2 external-workers spec then made a
  resolved `role-worker` provider the automatic-default BUILDER route. A
  profile whose `role-worker` resolves to a provider (not `claude-only`)
  now routes eligible non-security tasks to that provider by default, with
  no per-task `provider:` field required — subject to the sensitive-domain
  carve-out (BUILDER stays Claude) and the §1a provider gate layers. See
  the migration notice below for the one-time acknowledgment warning.
  **R1 migration notice — 2026-07-22 (bm-upgrade-migration-warning,
  `docs/specs/2026-07-22-phase2-external-workers.md`):** once Phase 2's R1
  automatic-default ROUTE semantics ship, `role-worker` stops being inert
  — a profile whose `role-worker` resolves to a provider (not
  `claude-only`) becomes that task's DEFAULT builder route with no
  per-task `provider:` field required. A profile that set
  `role-worker: <provider>` before Phase 2 shipped did so when the key
  had no dispatch effect; that pre-existing assignment would otherwise
  silently start routing eligible non-security tasks externally the
  moment automatic-default lands. `tools/validate_config.py`'s
  `validate_profile()` surfaces a one-time WARNING (never an error) for
  any `role-worker: <provider>` profile it validates, naming this
  semantic change, until a machine-local acknowledgment marker —
  `.forge/.trust-providers/role-worker.<provider>.migration-ack.local`,
  same gitignored, never-committed family as the per-provider TOFU trust
  markers and the pilot-clearance marker above — exists for that exact
  role-worker/provider pair. The marker is never written automatically;
  a human (or `/forge:settings`) creates it only after reviewing the
  assignment. Once present, the warning is suppressed for good for that
  pair — it is not re-nagged on every future validate run.
