...
**Renderer of record (fg-a10214):** `tools/status.py` is the canonical
renderer for `/forge:status` — a pure data query over task frontmatter, not
an LLM generation task, so `commands/status.md` runs it and prints its
stdout verbatim instead of loading this skill. This section is that
script's NORMATIVE spec: `tools/status.py` must implement exactly what's
defined below, so the two can never diverge. The LLM-rendering path
through this file remains live for two cases only: (1) the bare NL "what's
queued" ask (no command, no script invocation), and (2) `/forge:status`
falling back here if the script exits nonzero.
