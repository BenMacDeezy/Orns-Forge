...
many independent elements, or real-time interactive 3D (particles, geometry,
shaders, a rotatable model) — that's the signal to bring in a library; see
`lottie-rive-vector-animation`, `spring-physics-and-list-animation`, and
`webgl-react-three-fiber` for those cases specifically.
