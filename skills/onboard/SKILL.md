---
name: onboard
description: Set up Forge end to end in any repository — init .forge/, build the map, seed the constitution and forge.md, run a scout pass, and generate a root AGENTS.md bridged into Claude Code's own context via a CLAUDE.md @import. Use on /forge:onboard, or offer it when working in a repo that has no .forge/ directory.
---
...
6a. **Bridge `AGENTS.md` into Claude Code's own context.** Claude Code does
   NOT auto-load `AGENTS.md` — it only auto-loads `CLAUDE.md` (root,
   `.claude/CLAUDE.md`, nested-directory `CLAUDE.md`, `.claude/rules/*.md`).
   Without this step `AGENTS.md` is invisible to Claude Code sessions in this
   repo, which is exactly the gap that let a whole session run ad hoc instead
   of through Forge (2026-07-18 self-audit). If no root `CLAUDE.md` exists,
   create one from `references/claude-md-import-template.md` (an `@AGENTS.md`
   import line under a "## Forge" heading — Claude Code's documented
   `@path` import syntax). If a `CLAUDE.md` already exists, do NOT overwrite —
   idempotent like step 6: if it already contains an `@AGENTS.md` import or a
   "## Forge" section (a prior onboard), report it and change nothing; else
   append the same marked "## Forge" section from the template so the
   existing file's content is preserved.
7. **Report** — created vs. already-present per file (including whether
   `CLAUDE.md` was created, appended-to, or already had the Forge import), the
   scout shortlist, map status, whether discovery ran, and the next command
   (`/forge:add`, `/forge:spec`, or `/forge:start`).

## Safety

- Existing `forge.md`, `constitution.md`, `AGENTS.md`, `CLAUDE.md`, and map
  files are never clobbered — step 6a only ever creates a CLAUDE.md that
  doesn't yet exist, or appends its marked section to one that does.
- Onboard installs nothing and commits nothing on its own beyond the `.forge/`
  scaffolding, `AGENTS.md`, and the `CLAUDE.md` Forge-import bridge; the scout
  shortlist is advisory and install-free.
...
6. Generate a minimal AGENTS.md (project intent + "gates pending first code"),
   then bridge it into Claude Code's context the same way as step 6a above
   (create or append-to a root `CLAUDE.md` with the `@AGENTS.md` import) — an
   empty repo needs this exactly as much as an existing one.
