...
the template with `id: <readable-kebab-case-name>` describing the work
(e.g. `database-migration`, `color-refactor`; 3-40 chars,
`[a-z0-9][a-z0-9-]*[a-z0-9]`, no `--`) per spec-f0c2 Amendments item 2
(2026-07-20 ratification) -- checked unique against every id in
`.forge/specs/` (regenerate/pick a different name on collision), `status:
draft`, `approved-date: null`. Legacy `spec-<hex>` ids already on disk are
...
approval note in the Changelog. Never self-approve. This gate is a FLOOR —
unconditional regardless of the active operator profile
(`skills/kernel/references/profile-wiring.md`, "Floor enforcement"); a
profile may additionally mark the kernel's own PLAN step as a pause
point, but never removes or auto-answers this gate. Then validate:
...
One composition point, citing `skills/kernel/references/provider-judges.md`
(NORMATIVE) for mechanics — this pipeline defines no provider dispatch code
of its own. WHEN the active profile's `role-plan-refuter` resolves to
`codex`, the Architect-plan refuter second opinion
(`docs/conventions/verification.md`, "Architect-plan refuter") MAY be
dispatched to codex at its judgment tier instead of a second Claude
architect spawn, subject to `provider-judges.md`'s trust gate and
graceful-degrade. WHEN the active profile's `role-spec-review` resolves to
`codex`, step 2's drafted spec gets an additional codex spec-review pass
before step 3's clarification resolution — an advisory pass only, never a
gate: the human approval gate at step 5 remains the one authoritative
floor regardless of which tier reviewed the draft. Per the KERNEL RULING
`provider-judges.md` records, `role-spec-review` MAY run at codex's
mechanical tier — it is not bound by the co-verifier/plan-refuter
equal-or-higher floor that governs `role-plan-refuter` above.
