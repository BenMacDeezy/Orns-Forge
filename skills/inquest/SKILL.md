...
Each role is a single-purpose spawn contract, dispatched as a GENERIC
read-only agent — no dedicated roster `agentType`, same convention
`docs/conventions.md`, "Finder dispatch has no dedicated agentType," already
establishes for report-task finders ("the harness's general-purpose agent
under a kernel-declared read-only contract — not a roster `agentType`").
