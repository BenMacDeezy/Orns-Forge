...
**Model selection — ASK FIRST, every pass.** Before the first FINDER spawn,
ask the human which model to run the tribunal on — one `AskUserQuestion`,
per the "Asking the user questions" convention. The three roles are
READ-ONLY (Read/Grep/Glob/Bash), so any read-capable model is valid; there
is no fixed model here. Offer the routinely-available Claude tiers
(`haiku`, `sonnet`, `opus`, `fable`) and allow "Other" for a provider model
(e.g. `codex/gpt-5.6-sol`). Apply the chosen model to all three roles unless
the human names a per-role split — with ONE invariant that always holds:
the REFUTER's model stays **equal-or-higher** than the FINDER's (below).
Two honest routing facts the choice must respect:

- A **Claude tier** dispatches each role via its normal Agent-tool subagent
  (`forge-finder` / `forge-refuter` / `forge-judge`) at that model — works
  immediately, nothing to enable.
- A **provider model** (codex, etc.) dispatches each read-only role via
  `skills/kernel/references/provider-judges.md`'s Phase-1 read-only shape
  (`codex exec --sandbox read-only`), and REQUIRES the provider gates
  (`providers` Feature + the provider's toggle — both off by default) and
  the one-time per-provider trust confirmation to already be in place. This
  skill NEVER flips those itself (they are a deliberate human step); if they
  are not in place, say exactly that and fall back to a Claude tier.

When the human gives no answer (a headless / standing-consent pass), fall
back to the **default tiers** below.

**Default tiers (the fallback when no model is chosen):**
