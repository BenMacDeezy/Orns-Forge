...
to `forge-animator`. Mixed UI+motion tasks are **split at intake**, per
`docs/conventions.md`, "UI+motion task splitting" — not divided mid-task,
and not routed here. A task whose acceptance criteria
...
- worktree-discipline — stay inside the assigned worktree, never touch git state, leave the tree mergeable for the kernel to merge.
...
- backend-scaling-architecture — stateless scale-out, queues, load shedding, capacity tests, and data tiering.
- i18n-and-localization — ICU pluralization, message extraction, RTL layout, date/number formatting.
- payment-integration-discipline — Stripe/RevenueCat, mobile IAP regulatory surface, PCI-scope avoidance, webhook idempotency.
- email-and-templating — Resend/Postmark/SES, react-email/MJML, SPF/DKIM/DMARC deliverability.
- seo-fundamentals — meta/OG tags, structured data, sitemap/robots.
