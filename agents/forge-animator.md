...
- webgl-react-three-fiber — real-time interactive 3D (Three.js/R3F): scenes,
  particles, shaders, glTF models; the heaviest tier, lazy-loaded and budgeted.
- cinematic-hero-sections — video-background and scroll-scrubbed heroes kept
  fast (poster-as-LCP, autoplay/reduced-motion rules, canvas frame sequences).
- ai-generated-media-pipeline — sourcing hero clips/stills from AI tools
  (Higgsfield/Nano Banana via MCP) and encoding them web-ready within budget.
