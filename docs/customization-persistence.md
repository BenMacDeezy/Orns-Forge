...
| Banner launcher shim (örn banner) | User space — PowerShell `$PROFILE` (both `powershell`/`pwsh` if installed), `~/.bashrc`/`~/.zshrc`, a Windows `cmd.exe` shim directory (`~/.local/shims/claude.bat`), and an HKCU `AutoRun` registry value; a duplicate hand-wired hook in `~/.claude/settings.json` is also deregistered on install/uninstall | Byte-for-byte unchanged across `/forge:update` for the filesystem targets; the contract's own "User space" definition names this shim as its worked example even though several of these concrete targets (the PowerShell profile, `.bashrc`, the registry `AutoRun` value) sit outside the literal `~/.claude/...` path — they are still resolved via `Path.home()`/`$HOME`, outside any project working tree, and untouched by `/forge:update`, the properties the tier is defined by | Shipped — `/forge:banner install` \| `uninstall` \| `status` | `docs/conventions/config-and-features.md`, "Customization persistence contract" (names the örn banner shim as its user-space example); `tools/banner_install.py` |
