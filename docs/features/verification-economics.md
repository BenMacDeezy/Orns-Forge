...
cut its cost, honestly, with real numbers from the project's internal audit
rather than marketing language. Every figure below is either traced to that
audit or explicitly labeled an estimate.
...
## Verification modes and the grouped floor

Every completed task receives real adversarial verification. When 2-4
related tasks are ready, one verifier dispatch may cover the coherent group,
with a per-task verdict for each; groups can run in parallel with builds.
The low-risk sub-class is still a real spawned verifier, not an inline
exception. After an initial pass and one delta re-verify, unresolved judgment
escalates to a human blocker; cosmetic/P3 work is fixed inline. Full rules:
[`docs/conventions/verification.md`](../conventions/verification.md).

1. **Verifier spawn** (the default). `forge-verifier` (or
...
2. **Kernel synthesis** (report tasks only). A read-only "finder" task's
...
from a future run reaches the runtime protocol only through the same
human-gated change channel telemetry-driven recommendations already use,
never as a self-applied change.
