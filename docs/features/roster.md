...
and run charters under a twenty-fourth persona, **örn** — the orchestrator
is not backed by an `agents/*.md` file.

Seven agents are **judges** — read-only, never edit, rendering a verdict on
someone else's work or claim: Vera, Iris, Rook, Aegis, Lex, Foil (refutes
one inquest finding), and Gavel (weighs an inquest pass's full record and
routes it). Hound is also read-only but is not a judge — it proposes
findings, it doesn't verify anyone's work.
