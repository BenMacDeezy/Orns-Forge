...
Response to `.forge/queue/tasks/onboard-offer-nudge.md`: a fresh project
(no `.forge/`) once ran an entire PRD/court/plan cycle on another
workstation without the CLAUDE.md "offer /forge:onboard once when
substantial dev work begins" clause ever firing — a loud competing
SessionStart hook outweighed the quiet judgment-based instruction. Adds
`hooks/scripts/onboard-nudge.sh`, a fail-silent `SessionStart` hook on
the same `startup|resume` matcher `update-nudge.sh` uses
(`hooks/hooks.json`), making the offer mechanical instead of relying on
conversational judgment alone.

**What it checks.** The hook fires only in the CURRENT project directory,
and only when ALL of: the directory is a git repo; it has substantial
source content (a recognized project manifest — `package.json`,
`pyproject.toml`, `Cargo.toml`, `go.mod`, or a root-level `*.sln` — or
at least 10 tracked files with a common source-code extension); it has NO
`.forge/` directory; and no dedupe marker for that repo has been written
yet today. On a match it prints exactly ONE advisory
`hookSpecificOutput`/`additionalContext` line offering `/forge:onboard`,
then writes the dedupe marker.

**Never onboards, installs, or writes into the target repo.** Offering is
the entire behavior (trust boundary: Forge acts only on human intent, same
doctrine `docs/conventions/trust-and-security.md` states for every other
onboarding surface). The dedupe marker itself is never written inside the
target repo either — the repo by definition has no `.forge/` at the point
this hook fires — it lives in machine-local temp state outside any repo,
one file per project-path+day (keyed on the project directory, not the
git toplevel — see the speed-order note below), mirroring
`tools/update_check.py`'s `_cache_path()` convention. `FORGE_ONBOARD_NUDGE_STATE_DIR` overrides the
state directory (test injection point).

**Opt-out Feature.** `onboard-nudge` (default `on`), read from the
PLUGIN's own `.forge/forge.md` — never the target project's, which has no
`.forge/` here by definition — with the same bullet-tolerant,
case-insensitive `<toggle>: off` regex shape `tools/banner.py`'s
`hook_mode()` uses for `startup-banner`.

**Fail-silent, no network, advisory only.** Any error or uncertainty along
the way -> exit 0 with no stdout, matching every other Forge SessionStart
hook (`hooks/hooks.json`'s top-level description). The hook never blocks,
never repeats within the same repo+day, and never denies — it only ever
either prints one line or prints nothing.

**Single voice + speed order (2026-07-20 verify fixes).** This hook is
the ONLY surface that prints the no-`.forge/` onboard offer:
`session-start-inject.sh`'s older ungated line was removed in the same
change (two hooks printing it would double-voice every session forever —
the old line had no dedupe at all). And the dedupe-marker EXISTENCE check
runs BEFORE the `git ls-files` heuristic scan — and before every `git`
subprocess entirely, which is why the marker is keyed on the project
directory rather than the git toplevel (resolving the toplevel costs two
`git rev-parse` calls, measured ~100ms each) — so a repeat call on an
already-nudged repo exits in marker-stat time with zero git calls; the
marker WRITE stays after the heuristic actually fires, so a repo that
grows substantial later the same day can still earn its nudge.

## Mobile routing (forge-mobile pair) — 2026-07-21

Origin: `.forge/queue/tasks/forge-mobile-agent.md` — forge-ui's identity is
web-shaped (rendered browser UI, Core Web Vitals) and its verifier
screenshots via a browser MCP, which does not work for React Native/Expo
output. This section routes RN/Expo/mobile-shaped work to a dedicated pair
instead of forcing it through forge-ui.

**Routing rule.** WHEN a task is React Native / Expo / mobile-shaped —
components, screens, navigation, native-module boundaries, Metro,
platform-adaptive (iOS/Android) patterns, mobile performance budgets
(startup time, JS-thread frame drops, list virtualization) — THE SYSTEM
SHALL route it to `agents/forge-mobile.md` (persona Roam), not `forge-ui`.
`forge-ui` stays scoped to web-shaped UI (rendered browser output, Core Web
Vitals); it never absorbs mobile identity, and `forge-mobile` never absorbs
web identity — the two are siblings, not a superset/subset.

**Verification rule.** WHEN forge-mobile work is verified, THE SYSTEM SHALL
use `agents/forge-mobile-verifier.md` (persona Lens), gating `forge-mobile`
the same way `forge-ui-verifier` (Iris) gates `forge-ui`: judges only, never
fixes, adversarial render-and-observe verification, never a code-only
verdict when a device surface is available.

**Observation surface.** Lens observes rendered output on a simulator or
emulator — Android emulator via `adb` (`adb exec-out screencap -p` for
screenshots, `adb shell` for interaction, `logcat` for red-screen/JS
errors), or iOS Simulator via `xcrun simctl` (macOS hosts only) — never a
web browser. A browser is not a substitute observation surface for mobile
output, structurally, regardless of how similar the rendered pixels look.

**Honest-degradation rule.** WHEN no simulator/emulator is available on the
host (e.g. Windows without a running Android emulator, or iOS anywhere
off-macOS), THE SYSTEM SHALL degrade honestly: gate-level + static
verification only, with an explicit "visual verification UNAVAILABLE on
this host" flag carried in the verdict — never a fabricated or
browser-substituted visual pass.

**Skills.** `forge-mobile` attaches `react-native-foundations` (navigation,
safe areas, platform conventions, mobile a11y, StyleSheet/NativeWind),
`react-native-performance` (startup time, JS-thread frame drops, list
virtualization, New Architecture/Hermes), and `react-native-motion-gestures`
(Reanimated worklets/shared values/layout animations, Gesture Handler
composition, worklet/JS-thread interop pitfalls) for screen-local motion and
gesture work, plus `react-performance` where general React render discipline
applies on native. `forge-mobile-verifier` checks output against the same
performance/foundations skills and additionally attaches
`mobile-visual-testing` (adb/xcrun-simctl capture, Maestro flows on the free
local CLI, screenshot-diff conventions, mobile-specific flake sources) as
the HOW behind its render-and-observe step; neither agent carries
`forge-ui`'s web-only skills (`core-web-vitals-for-ui`,
`tailwind-v4-composition-patterns`, `component-system-shadcn-radix`).

## Mobile routing — RN-web ruling — 2026-07-21

Human-ratified ruling closing the edge the forge-mobile-agent verify
recorded: routing follows the **rendered surface**, never the source
framework. A React Native / Expo codebase does not make a task
mobile-shaped by itself:

- Deliverable renders **in a browser** (Expo web export, React Native
  Web target) -> routes to `forge-ui`, verified by `forge-ui-verifier`
  on the browser surface — attach the RN skills alongside forge-ui's web
  skills when the code is RN-flavored.
- Deliverable renders **on a device** (emulator/simulator/physical) ->
  routes to `forge-mobile`, verified by `forge-mobile-verifier`.
- A task whose acceptance criteria span BOTH surfaces from one shared
  codebase is **split before dispatch** (same doctrine as the UI+motion
  task-splitting convention): one device-verified task and one
  browser-verified task, each judged on its own surface. One task is
  never closed with only one of its two declared surfaces verified.

## Sensitive-domain build carve-out — 2026-07-22 (owner-directed)

Refines the provider-first routing preference (the owner's 2026-07-22
pivot toward external-provider workers) without weakening any floor:
some things Claude just builds.

- WHEN a task's scope touches a domain on the forge-security NAMED
  trigger list (`docs/conventions/verification.md`, "Verification
  economics" panel policy: auth/token/secret, money/payment,
  cookie/storage write, raw-HTML, form/redirect, untrusted-input
  parsing, new dependency) — or the provider gate machinery itself
  (trust markers, toggles, pilot gates, dispatch caps) — THE SYSTEM
  SHALL default the BUILDER to an in-harness Claude agent, even where a
  provider-first preference is active. The carve-out binds the builder
  role only: external providers remain welcome on the same task as
  judges, co-verifiers, and second opinions (cross-model review of
  sensitive work is encouraged, authorship of it is not delegated by
  default).
- WHEN a human wants a specific sensitive build routed to a provider
  anyway, THE SYSTEM SHALL require an explicit per-task routing
  override (the provider-qualified form) — the carve-out is a default,
  not a floor, but only a human-written override line crosses it; the
  orchestrator never crosses it on its own judgment.
- WHEN the carve-out fires, THE SYSTEM SHALL say so in the dispatch
  label's routing rationale (one clause, e.g. "builder=claude
  (sensitive-domain carve-out: auth)") so the record shows WHY the
  provider-first preference was not applied.

## Attribute routing matrices — 2026-07-22 (spec cross-model-consensus)

Implements `docs/specs/2026-07-22-cross-model-consensus.md` AC E —
NORMATIVE, a documentation-only default: no routing code changes here, the
ROUTE step already resolves `role-worker`/`role-co-verifier` per profile;
this section documents the DEFAULT those resolutions should express absent
an explicit override. THE SYSTEM SHALL maintain TWO separate default-
routing matrices, never one shared table, because the sensitive-domain
carve-out (above) binds the BUILDER role only and explicitly welcomes
provider JUDGES.

**BUILDER matrix** (WHEN the kernel's ROUTE step assigns a BUILDER role and
more than one route is otherwise eligible):

| Shape | Default route |
|---|---|
| Mechanical / speed-shaped (bulk sweeps, codemods, formatting, high-volume low-judgment work) | codex, `codex-tier-mechanical` or `codex-tier-balanced` per `provider-judges.md` section 9's tier map, checkpoint-gated |
| Deep-judgment / security-shaped (architecture, root-cause debugging, auth/money/PII/destructive-ops/untrusted-input domains) | Claude, per the sensitive-domain carve-out's existing BUILDER default (above) — this table does not change that default, it names WHY the "deep-judgment" row routes there |
| Balanced (well-specified building, neither bulk-mechanical nor sensitive-domain) | tie-break within the active profile's allowed pool for `role-worker`, per the task-shape tie-break rule below |

**JUDGE matrix** (WHEN the kernel's ROUTE step assigns a JUDGE role —
verifier, co-verifier, plan-refuter, spec-review — and more than one route
is otherwise eligible; the sensitive-domain BUILDER carve-out does NOT
apply to this matrix):

| Shape | Default route |
|---|---|
| Mechanical / speed-shaped judge passes (spec-review at mechanical tier per the existing KERNEL RULING, `provider-judges.md` section 3) | codex, `codex-tier-mechanical` per section 9 |
| Judgment-tier judge passes (plan-refuter, adversarial/co-verifier on tier:full or sensitive-domain work) | either provider at `codex-tier-judgment` or Claude, resolved by the active profile's `role-plan-refuter`/`role-co-verifier` assignment — the carve-out does not block a codex JUDGE here, only a codex BUILDER |

Reference ONLY tier keys (`codex-tier-mechanical`, `codex-tier-balanced`,
`codex-tier-judgment`) in both matrices, per `provider-judges.md` section
9's tier map — NEVER a model slug or nickname (no "luna," "terra," or
similar), so a future re-pin of section 9 updates both matrices' effective
meaning automatically without an edit to this section.

**Precedence chain**, applied in this exact order for BOTH matrices:

1. Human-written routing override (explicit routing note on the task)
   always wins — but when that override crosses the sensitive-domain
   carve-out (step 2), only a single-use, un-forgeable authenticated-human
   provenance envelope (`skills/kernel/references/carve-out-provenance.md`,
   R2) authenticates the crossing; a bare routing note does not cross.
2. Sensitive-domain BUILDER carve-out (above) — applies to the BUILDER
   matrix only, forces Claude BUILDER regardless of shape.
3. The four-layer provider gate (`provider-judges.md` section 1a) plus
   pilot eligibility (grok/antigravity excluded until their pilot clears)
   — a route that fails any gate layer is not eligible, regardless of
   what the matrix would otherwise say.
4. The active profile's resolved `role-*` assignment
   (`operator-profiles.md`) — if the profile already pins a role to a
   specific provider, that resolution is used directly; there is no tie
   left for the matrix to break.
5. Task-shape tie-break WITHIN the profile's allowed pool — ONLY when
   step 4 leaves more than one eligible provider does the matrix's shape
   row (mechanical/deep-judgment/balanced) decide.

"Checkpoint budget pressure" is REMOVED as a tie-break input: checkpoints
(`provider-dispatch-checkpoint-every`) are a VISIBILITY mechanism (the
human is shown a running tally and may object), not a routing pressure
signal — dispatching continues by default at a checkpoint unless the human
explicitly objects, so there is no deterministic "pressure" state for a
tie-break to consult.

WHEN either matrix's default would route a sensitive-domain BUILD to
codex, THE SYSTEM SHALL apply the sensitive-domain carve-out instead
(Claude BUILDER by default; codex may still judge/co-verify per the JUDGE
matrix) — both matrices are written to already agree with the carve-out,
and this section exists to make the precedence explicit and testable, not
to introduce a new conflict-resolution rule. The table breaks ties only,
never overrides a floor: every existing floor named in `docs/specs/
2026-07-22-cross-model-consensus.md`'s "Floors carried forward unmoved"
section — the `providers` Feature, per-provider toggle, TOFU, checkpoint
budget, the `tier: full` spec-approval gate, the sensitive-domain
carve-out, local-CLI-only credentials, and the pilot gate on
grok/antigravity — stays exactly where it is; this section neither moves
nor loosens any of them.

## Task|Agent PreToolUse hook consolidation — 2026-07-23

`budget-guard.sh` and `agent-provenance-flag.sh` were registered as two
separate PreToolUse commands on the same `Task|Agent` matcher, so every
agent dispatch paid TWO bash process spawns. On Windows (Git Bash) a spawn
costs tens to hundreds of milliseconds, and with `max-parallel-tasks: auto`
the kernel fires a whole wave of dispatches at once — the exact hot path
where that doubling surfaces as hook-timeout noise. Both are now invoked
through a single registered hook, `hooks/scripts/task-dispatch.sh`, which
reads stdin once and runs both checks in ONE process.

**Contract.** `task-dispatch.sh` sources both scripts and calls
`forge_agent_provenance` first, then `forge_budget_guard` — both
unconditionally. That ordering preserves the pre-merge behavior where two
independent hooks each saw every dispatch: a budget denial must still
leave a provenance line. Only `budget-guard.sh` writes stdout (its deny
decision); the provenance flag is append-to-log only, so the harness still
receives exactly one JSON decision object and there is no output-merge
conflict.

**Source-safe rule.** Each script keeps its logic in a function
(`forge_budget_guard` / `forge_agent_provenance`) and self-runs only from a
`[ "${BASH_SOURCE[0]}" = "$0" ]` guard, so both remain independently
executable and independently tested. Inside those function bodies every
early bail MUST be `return 0`, never `exit` — an `exit` in a sourced
function kills the dispatcher and silently skips whichever check runs
second. `tools/test_hooks.py` pins this statically as well as behaviorally.

**Unchanged.** `budget-guard.sh` remains the only hook allowed to block a
dispatch, and `staleness-flag.sh` (matcher `Bash`) stays a separate
registration — it is on a different matcher and shares no input.

## No-worktree degraded mode — 2026-07-24 (owner-directed)

Owner-directed after a live session on a OneDrive/Windows repo where
`git worktree add` returns exit 0 but materializes empty/un-synced trees.
The session hand-forced sequential + Claude-only and wrongly concluded a
codex worker "can't run without worktree isolation." That was an
interpretation bug: provider-judges.md §7.2 says a mutating provider
worker gets "the SAME isolation a Claude `forge-worker` would get" — and
on the sequential path a Claude worker gets NONE (it writes in-place to
the main tree). So codex mirrors that: in-place, no worktree.

**Canonical mechanics:** `skills/kernel/references/degraded-worktree-mode.md`
(NORMATIVE). Summary of the contract:
- **SYNC probe.** Once per session the kernel probes whether worktrees
  actually work (create a throwaway worktree, assert it materialized a
  non-empty tree, remove it — an exit-0-but-empty result on a synced
  filesystem still counts as failure). On failure the session is DEGRADED
  and the run charter/session report says so verbatim. A human may also
  force it via forge.md `worktrees: off`.
- **Degraded = sequential + in-place.** GATE's wave-parallel and
  shard-eligibility predicates FAIL CLOSED (no isolation → no two mutating
  workers in one tree). This is the ONE condition that overrides
  parallel-first dispatch, and only because the safety precondition is
  absent, not as a preference.
- **Both worker kinds run in-place, identically.** A codex worker launches
  `--sandbox workspace-write` with cwd at the repo root (workspace-write is
  a codex sandbox, not a git worktree); §8 skill materialization writes to
  `.forge-dispatch/` at the repo root and the §8 INTEGRATE-time exclusion
  becomes load-bearing; INTEGRATE is the ordinary single-task in-place
  commit; a bounce reverts the bad diff in-place before the next task.
  Hard Rule 4 is unchanged — the worker never touches `.forge/`.
- **Verify is builder-agnostic.** A visual task routes to Claude
  `forge-ui-verifier` (Playwright gate) no matter who built the diff, so
  **codex-build + Claude-visual-verify** is a first-class combination.
  The equal-or-higher-model-tier floor on the verifier is unchanged.
