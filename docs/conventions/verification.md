...
> Amends: "Idle-wait discipline — watchdog amendment — 2026-07-20" and
> "Watchdog thresholds — 2026-07-20" (above).

Owner-directed after a live session where a single sequential worker (a
palette+tokens task) ran 30+ minutes and 80+ tool calls with no dispatch
progress and no watchdog flag. Root cause: the kernel dispatched the
worker SYNCHRONOUSLY — in the foreground, blocking the loop — because
"the sequential flow needs its diff before verification." The watchdog
amendment above only runs `tools/watchdog.py` "on the one long fallback
wakeup the discipline allows"; a foreground-blocked kernel never idle-
waits, so no wakeup fires, so the watchdog NEVER RUNS, so the runaway is
caught only when the human interrupts.

**Sequential ≠ synchronous.** Sequential means one worker at a time,
awaited before the next. Synchronous means the loop blocks in the
foreground. They are orthogonal, and conflating them defeats supervision.

**Rule.** A sequentially-dispatched worker — including every worker in
no-worktree DEGRADED mode (`skills/kernel/references/
degraded-worktree-mode.md`) — SHALL be dispatched in the BACKGROUND and
awaited via its completion notification, NEVER foreground-blocked. The
kernel still consumes exactly one worker's diff before verifying and
before the next dispatch (the sequential guarantee is unchanged), but it
does so from an idle-wait, not a blocked call. This keeps the one allowed
fallback wakeup alive, on which the kernel runs `tools/watchdog.py`
against the lone in-flight worker exactly as it already does for a
parallel batch. A worker the watchdog flags HUNG or RUNAWAY is killed and
bounced (attempt++), same as any watchdog-flagged worker; the bounce
reverts in-place per degraded-mode rules when there is no worktree to
discard.

**New threshold — `runaway-commands` (default 50).** Adds a hard
per-worker command/tool ceiling to "Watchdog thresholds — 2026-07-20"'s
set: when a worker's transcript records ≥ `runaway-commands` `$ ` command
lines, `tools/watchdog.py` flags it RUNAWAY even if no command repeats
identically — the 80-distinct-tool-calls churn the identical-repeat check
alone misses. Overridable per-project via `.forge/forge.md`'s `## Features`
as `- watchdog-runaway-commands: <n>`, exactly like every other
`watchdog-<name>` threshold; a missing or malformed value keeps the
default. This composes with, and does not replace, `runaway-repeats`
(retry-loop shape) or `runaway-bytes` (transcript size) — any one firing
is a RUNAWAY.
