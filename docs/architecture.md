...
- **Roster** (`agents/*.md`) — 20 routed agents plus the örn orchestrator
...
flowchart TD
    ...
    VERIFY -->|"trivial tier"| GI["mode 1: gates inline<br/>no separate spawn"]
    VERIFY -->|"standard/full"| VJ["mode 2: forge-verifier / forge-ui-verifier<br/>equal-or-higher tier<br/>Low-risk sub-tier when it qualifies"]
    VERIFY -->|"read-only report task"| KS["mode 3: kernel synthesis<br/>of a finder's report"]
    GI --> INTEGRATE
    VJ -->|"PASS"| INTEGRATE
...
Twenty thin slash-command entry points under `commands/*.md`, each invoking
