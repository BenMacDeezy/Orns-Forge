...
class TestValidateAll(unittest.TestCase):
    ...
    # -- BUG 1: UnicodeDecodeError must not crash the collision-id scan --

    def test_utf16_task_file_does_not_crash_collision_check(self):
        tmp = self._build_repo(VALID_TASK, VALID_SPEC)
        tasks_dir = pathlib.Path(tmp, ".forge", "queue", "tasks")
        (tasks_dir / "fg-3fa9-example.md").write_bytes(
            VALID_TASK.encode("utf-16"))
        code, out = self._run_in(tmp)  # must not raise
        self.assertEqual(code, 1)
        self.assertIn("2 file(s) checked", out)

    # -- BUG 3: one sub-validator crashing on one file must not kill the
    # whole batch --

    def test_one_crashing_file_does_not_kill_whole_batch(self):
        tmp = self._build_repo(VALID_TASK, VALID_SPEC)
        tasks_dir = pathlib.Path(tmp, ".forge", "queue", "tasks")
        (tasks_dir / "fg-4444-second.md").write_text(
            VALID_TASK.replace("id: fg-3fa9", "id: fg-4444"), encoding="utf-8")
        real_validate = validate_all.validate_task.validate

        def crashing_validate(path, **kwargs):
            if "fg-3fa9" in str(path):
                raise RuntimeError("boom")
            return real_validate(path, **kwargs)

        with mock.patch.object(validate_all.validate_task, "validate",
                               side_effect=crashing_validate):
            code, out = self._run_in(tmp)  # must not raise
        self.assertEqual(code, 1)
        self.assertIn("internal validator error", out)
        self.assertIn("boom", out)
        # the other two files (fg-4444 task + spec) were still checked
        summary_lines = [l for l in out.splitlines() if "file(s) checked" in l]
        self.assertIn("3 file(s) checked, 1 error(s)", summary_lines[0])

    # -- BUG 12: validate_all must pass warnings= through to validate_task
    # and validate_memory, matching validate_config --

    def test_task_warnings_aggregated_in_combined_output(self):
        trivial_bug_task = VALID_TASK.replace(
            "tier: standard", "tier: trivial").replace(
            "title: Add rate limiting", "title: Fix login crash")
        tmp = self._build_repo(trivial_bug_task, VALID_SPEC)
        code, out = self._run_in(tmp)
        self.assertEqual(code, 0)
        self.assertIn("WARNING", out)
        self.assertIn("bug fixes normally need tier: standard", out)

    def test_memory_bleed_warnings_aggregated_in_combined_output(self):
        # Craft-memory bleed warnings only fire in the plugin-level craft
        # store (parent dir named "memory" whose own parent isn't ".forge"),
        # so build that shape directly and pass it as an explicit path.
        tmp = tempfile.mkdtemp()
        craft_memory = pathlib.Path(tmp, "plugin-root", "memory")
        craft_memory.mkdir(parents=True)
        bleeding = VALID_MEMORY.replace(
            "A whole-file substring test",
            "Ask hockeyben about this -- a whole-file substring test")
        fact_path = craft_memory / "bleed-fact.md"
        fact_path.write_text(bleeding, encoding="utf-8")
        code, out = self._run_with_args(tmp, [str(fact_path)])
        self.assertEqual(code, 0)
        self.assertIn("WARNING", out)
        self.assertIn("hockeyben", out)
    ...
    # -- BUG 16: routing by parent-directory name alone must not force-route
    # unrelated files into the rigid Forge schema --

    def test_unrelated_specs_named_dir_not_force_routed(self):
        tmp = tempfile.mkdtemp()
        fake_specs_dir = pathlib.Path(tmp, "specs")
        fake_specs_dir.mkdir()
        unrelated = fake_specs_dir / "notes.md"
        unrelated.write_text("# just some notes, not a Forge spec\n",
                             encoding="utf-8")
        code, out = self._run_with_args(tmp, [str(unrelated)])
        self.assertEqual(code, 1)
        self.assertIn("cannot determine validator", out)

    def test_unrelated_tasks_named_dir_not_force_routed(self):
        tmp = tempfile.mkdtemp()
        fake_tasks_dir = pathlib.Path(tmp, "tasks")
        fake_tasks_dir.mkdir()
        unrelated = fake_tasks_dir / "notes.md"
        unrelated.write_text("# just some notes, not a Forge task\n",
                             encoding="utf-8")
        code, out = self._run_with_args(tmp, [str(unrelated)])
        self.assertEqual(code, 1)
        self.assertIn("cannot determine validator", out)

    # -- fg-a11034: `.forge` ancestry check in `_route_paths` must be
    # case-insensitive, matching validate_memory.py's `_craft_plugin_root` --

    def test_route_paths_case_insensitive_dotforge_task(self):
        tmp = tempfile.mkdtemp()
        tasks_dir = pathlib.Path(tmp, ".Forge", "queue", "tasks")
        tasks_dir.mkdir(parents=True)
        task_path = str(tasks_dir / "fg-3fa9-example.md")
        (tasks_dir / "fg-3fa9-example.md").write_text(VALID_TASK, encoding="utf-8")
        tasks, specs, memories, configs, unroutable = \
            validate_all._route_paths([task_path])
        self.assertEqual(unroutable, [])
        self.assertEqual(tasks, [task_path])

    def test_route_paths_case_insensitive_dotforge_spec(self):
        tmp = tempfile.mkdtemp()
        specs_dir = pathlib.Path(tmp, ".Forge", "specs")
        specs_dir.mkdir(parents=True)
        spec_path = str(specs_dir / "2026-07-17-example.md")
        (specs_dir / "2026-07-17-example.md").write_text(VALID_SPEC, encoding="utf-8")
        tasks, specs, memories, configs, unroutable = \
            validate_all._route_paths([spec_path])
        self.assertEqual(unroutable, [])
        self.assertEqual(specs, [spec_path])

    def test_uppercase_dotforge_task_routes_and_validates_via_main(self):
        tmp = tempfile.mkdtemp()
        tasks_dir = pathlib.Path(tmp, ".Forge", "queue", "tasks")
        tasks_dir.mkdir(parents=True)
        task_path = str(tasks_dir / "fg-3fa9-example.md")
        ...
        code, out = self._run_with_args(tmp, [task_path])
        self.assertEqual(code, 0)
        self.assertNotIn("cannot determine validator", out)
        summary_lines = [l for l in out.splitlines() if "file(s) checked" in l]
        self.assertIn("1 file(s) checked, 0 error(s)", summary_lines[0])
    ...
    def test_em_dash_message_does_not_crash_under_legacy_codepage(self):
        newer_task = VALID_TASK.replace(
            "updated: 2026-07-16\n---",
            "updated: 2026-07-16\nschema-version: 2\n---")
        tmp = self._build_repo(newer_task, VALID_SPEC)
