...
def _is_valid_id(val):
    """True if `val` is either a legacy hex id (spec-xxxx, grandfathered
...
def validate(path, warnings=None):
    """Validate one spec file. Returns the error list (unchanged contract).
    ...
    if "id" in fm and not _is_valid_id(fm["id"]):
        errors.append(
            f"bad id: {fm['id']!r} (want a legacy spec-xxxx hex id, or a "
        ...
        warnings.append(
            f"{path}: id {fm['id']!r} starts with the legacy 'spec-' prefix "
