...
class RunawayRepeatTests(unittest.TestCase):
    ...
    def test_distinct_commands_over_budget_flags_runaway(self):
        # The 2026-07-24 ceiling: a worker churning many DISTINCT commands
        # (the palette task that fanned out to 80 tool calls) is a runaway
        # the identical-repeat check alone never catches.
        with tempfile.TemporaryDirectory() as tmp:
            task_dir = pathlib.Path(tmp)
            transcript = task_dir / "fg-r220.transcript"
            lines = "".join(f"$ cmd{i}\nok\n" for i in range(12))
            transcript.write_text(lines, encoding="utf-8")
            tasks = [{
                "id": "fg-r220", "state": "active",
                "claimed_by": f"sess-ab12 @ {_iso(NOW - timedelta(hours=1))}",
            }]
            flags = watchdog.check_runaway(
                tasks, task_dir, 10 * 1024 * 1024, 5, command_budget=10)
            self.assertEqual(len(flags), 1)
            self.assertTrue(flags[0].startswith("RUNAWAY fg-r220:"))
            self.assertIn("ran 12 commands", flags[0])
            self.assertIn(">= 10 budget", flags[0])

    def test_commands_under_budget_not_flagged(self):
        with tempfile.TemporaryDirectory() as tmp:
            task_dir = pathlib.Path(tmp)
            transcript = task_dir / "fg-r221.transcript"
            transcript.write_text("$ a\nok\n$ b\nok\n$ c\nok\n",
                                  encoding="utf-8")
            tasks = [{
                "id": "fg-r221", "state": "active",
                "claimed_by": f"sess-ab12 @ {_iso(NOW - timedelta(hours=1))}",
            }]
            flags = watchdog.check_runaway(
                tasks, task_dir, 10 * 1024 * 1024, 5, command_budget=10)
    ...
    def test_command_budget_defaults_disabled_when_omitted(self):
        # Back-compat: callers that don't pass command_budget get the old
        # behavior (no ceiling), so nothing that omits it changes.
        with tempfile.TemporaryDirectory() as tmp:
            task_dir = pathlib.Path(tmp)
            transcript = task_dir / "fg-r222.transcript"
            transcript.write_text(
                "".join(f"$ cmd{i}\nok\n" for i in range(99)),
                encoding="utf-8")
            tasks = [{
                "id": "fg-r222", "state": "active",
                "claimed_by": f"sess-ab12 @ {_iso(NOW - timedelta(hours=1))}",
            }]
            flags = watchdog.check_runaway(tasks, task_dir, 10 * 1024 * 1024, 5)
            self.assertEqual(flags, [])

    def test_runaway_commands_default_present(self):
        self.assertEqual(watchdog.DEFAULTS["runaway-commands"], 50)
