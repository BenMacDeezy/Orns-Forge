...
class TestQueueStatsFrontmatterScoping(unittest.TestCase):
    """Bug: _queue_stats regex-searched the ENTIRE task file text for the
    first ^state:\\s*(\\S+) line (MULTILINE), not scoped to the YAML
    frontmatter block. A task with malformed/absent frontmatter state but a
    body sentence matching that pattern (e.g. in an Execution-plan
    paragraph) got misattributed to that state."""

    def test_body_text_matching_state_pattern_is_not_counted(self):
        import tempfile
        import pathlib
        with tempfile.TemporaryDirectory() as tmp:
            project = pathlib.Path(tmp)
            tasks = project / ".forge" / "queue" / "tasks"
            tasks.mkdir(parents=True)
            (tasks / "fg-test.md").write_text(
                "---\n"
                "id: fg-test\n"
                "title: Test task with no frontmatter state\n"
                "---\n"
                "\n"
                "## Execution plan\n"
                "\n"
                "state: active is the target once step 3 lands.\n",
                encoding="utf-8",
            )
            result = banner._queue_stats(cwd=project)
            self.assertIsNone(
                result,
                "a body sentence matching ^state:\\s*(\\S+) must not be "
                "counted as the task's real state -- only the frontmatter "
                "block (between the leading '---' delimiters) counts",
            )

    def test_real_frontmatter_state_still_counted_once(self):
        import tempfile
        import pathlib
        with tempfile.TemporaryDirectory() as tmp:
            project = pathlib.Path(tmp)
            ...
            (tasks / "fg-test.md").write_text(
                ...
                "---\n"
                "\n"
                "state: this decoy body line must not double-count.\n",
                encoding="utf-8",
            )
            result = banner._queue_stats(cwd=project)
...
class TestQueueStatsAsciiSafeSeparator(unittest.TestCase):
    """Bug: ascii_safe mode only swapped TAGLINE -> TAGLINE_ASCII; it did
    not sanitize the '·' (U+00B7 MIDDLE DOT) separator joining queue-state
    counts. Real stdout that can't encode non-ASCII crashed with
    UnicodeEncodeError despite ascii_safe existing specifically to prevent
    that."""

    def _project_with_two_states(self, tmp):
        import pathlib
        project = pathlib.Path(tmp)
        tasks = project / ".forge" / "queue" / "tasks"
        tasks.mkdir(parents=True)
        (tasks / "fg-a.md").write_text("---\nstate: ready\n---\n", encoding="utf-8")
        (tasks / "fg-b.md").write_text("---\nstate: active\n---\n", encoding="utf-8")
        return project

    def test_ascii_safe_queue_stats_has_no_middle_dot(self):
        import tempfile
        with tempfile.TemporaryDirectory() as tmp:
            project = self._project_with_two_states(tmp)
            stats = banner._queue_stats(cwd=project, ascii_safe=True)
            self.assertIsNotNone(stats)
            self.assertNotIn("·", stats)

    def test_non_ascii_safe_queue_stats_keeps_middle_dot(self):
        import tempfile
        with tempfile.TemporaryDirectory() as tmp:
            project = self._project_with_two_states(tmp)
            stats = banner._queue_stats(cwd=project, ascii_safe=False)
            self.assertIn("·", stats)

    def test_render_ascii_safe_output_is_strict_ascii_encodable(self):
        import tempfile
        import os as _os
        with tempfile.TemporaryDirectory() as tmp:
            project = self._project_with_two_states(tmp)
            ...
            try:
                _os.chdir(project)
                result = banner.render(color=False, small=True, ascii_safe=True)
            finally:
                _os.chdir(cwd)
            try:
                result.encode("ascii")
                ...
                self.fail(
                    f"render(ascii_safe=True) produced output that is not "
                    f"strict-ASCII-safe, despite ascii_safe existing "
                    f"specifically to prevent that: {e}"
                )


class TestQueueStatsBomPrefixedTaskFile(unittest.TestCase):
    """fg-a10933: _queue_stats read task files with plain encoding="utf-8"
    (not "utf-8-sig", unlike every other BOM-aware reader in this codebase --
    trust.py, validate_task.py, telemetry.py, queue_graph.py,
    banner_install.py). A BOM-prefixed task file (PowerShell's
    `Out-File`/`Set-Content` default) leaves a leading U+FEFF character,
    which makes _FRONTMATTER_RE's `\\A---` anchor fail to match, so the task
    silently vanished from the queue-stats banner entirely."""

    def test_bom_prefixed_task_file_is_still_counted(self):
        import tempfile
        import pathlib
        with tempfile.TemporaryDirectory() as tmp:
            project = pathlib.Path(tmp)
            ...
            tasks.mkdir(parents=True)
            content = "---\nid: fg-test\nstate: ready\n---\n"
            # Write with a leading UTF-8 BOM, as PowerShell's Out-File /
            # Set-Content default encoding does.
            (tasks / "fg-test.md").write_bytes(content.encode("utf-8-sig"))
            result = banner._queue_stats(cwd=project)
            self.assertEqual(
                result, "1 ready",
                "a BOM-prefixed task file must still be counted correctly, "
                "not silently vanish from the queue-stats banner",
            )


class TestHookModeStartupBannerOffCaseInsensitive(unittest.TestCase):
    """Bug: the startup-banner: off opt-out regex had no re.IGNORECASE,
    unlike every other value-matching regex in this codebase. '- startup-
    banner: Off' or 'OFF' in forge.md failed to match, silently ignoring
    the user's opt-out."""

    def _run_hook_mode(self, forge_md_body):
        import tempfile
        import pathlib
        with tempfile.TemporaryDirectory() as tmp:
            project = pathlib.Path(tmp)
            ...
            (forge_dir / "forge.md").write_text(forge_md_body, encoding="utf-8")
            buf = io.StringIO()
            with patch("sys.stdout", buf), \
    ...
    def test_mixed_case_off_is_respected(self):
        out = self._run_hook_mode("# Forge config\n\n## Features\n- startup-banner: Off\n")
        self.assertEqual(out, "", "mixed-case 'Off' must suppress the banner just like 'off'")

    def test_uppercase_off_is_respected(self):
        out = self._run_hook_mode("# Forge config\n\n## Features\n- startup-banner: OFF\n")
        self.assertEqual(out, "", "uppercase 'OFF' must suppress the banner just like 'off'")

    def test_lowercase_off_still_respected(self):
        out = self._run_hook_mode("# Forge config\n\n## Features\n- startup-banner: off\n")
        self.assertEqual(out, "")


class TestSupportsColorNoColorPresence(unittest.TestCase):
    """Bug: os.environ.get("NO_COLOR") truthiness treated NO_COLOR="" (set
    but empty, as PowerShell/bash can produce) as falsy, failing to disable
    color. Per the NO_COLOR convention, mere PRESENCE of the variable (any
    value) should suppress color.

    Under pytest, sys.stdout.isatty() is already False, which independently
    forces _supports_color() to False and would mask the NO_COLOR bug -- so
    stdout is mocked as a tty here to isolate the NO_COLOR check itself.
    """

    def _fake_tty_stdout(self):
        fake = MagicMock()
        fake.isatty.return_value = True
        return fake

    def test_empty_no_color_value_disables_color(self):
        with patch("sys.stdout", self._fake_tty_stdout()), \
                patch.dict("os.environ", {"NO_COLOR": ""}), \
                patch("sys.argv", ["banner.py"]):
            self.assertFalse(banner._supports_color())

    def test_non_empty_no_color_value_disables_color(self):
        with patch("sys.stdout", self._fake_tty_stdout()), \
