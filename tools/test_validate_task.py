...
class TestProviderField(unittest.TestCase):
    """bm-provider-schema-field (docs/specs/2026-07-22-phase2-external-
    workers.md): optional `provider:` frontmatter field, validated against
    validate_config.PROVIDER_ENUM union {"claude-only"} (imported, never
    restated), plus fail-closed validation of near-miss keys."""
    ...
    def test_provider_absent_is_valid(self):
        self.assertNotIn("provider", VALID)
        self.assertEqual(validate(self._write(VALID)), [])

    # -- valid enum values accepted --

    def test_provider_codex_accepted(self):
        ok = VALID.replace("parallel-safe: true",
                           "parallel-safe: true\nprovider: codex")
        self.assertEqual(validate(self._write(ok)), [])

    def test_provider_grok_accepted(self):
        ok = VALID.replace("parallel-safe: true",
                           "parallel-safe: true\nprovider: grok")
        self.assertEqual(validate(self._write(ok)), [])

    def test_provider_antigravity_accepted(self):
        ok = VALID.replace("parallel-safe: true",
                           "parallel-safe: true\nprovider: antigravity")
        self.assertEqual(validate(self._write(ok)), [])

    def test_provider_claude_only_accepted(self):
        ok = VALID.replace("parallel-safe: true",
    ...
    def test_provider_bad_value_rejected(self):
        broken = VALID.replace("parallel-safe: true",
                               "parallel-safe: true\nprovider: gpt5")
        errs = validate(self._write(broken))
        self.assertTrue(any("provider" in e and "gpt5" in e for e in errs), errs)

    def test_provider_empty_value_rejected(self):
        # Verify-bounce V1 (P1): a PRESENT provider: key with an empty
        # value is a malformed override and must FAIL CLOSED, never be
        # silently treated as "no override" -- under automatic-default a
        # blank provider: must not slip through unvalidated.
        empty = VALID.replace("parallel-safe: true",
                              "parallel-safe: true\nprovider:")
        errs = validate(self._write(empty))
        self.assertTrue(any("bad provider" in e for e in errs), errs)

    # -- fail-closed near-miss key: the plural typo --

    def test_providers_plural_typo_rejected(self):
        broken = VALID.replace("parallel-safe: true",
                               "parallel-safe: true\nproviders: codex")
        errs = validate(self._write(broken))
        self.assertTrue(
            any("providers" in e and "near-miss" in e for e in errs), errs)

    def test_providers_plural_typo_rejected_even_with_valid_provider_value(self):
        # The near-miss key check fires independently of whatever value
        # accompanies it -- a typo'd key must never be silently absorbed as
        # a route override, regardless of its value's own validity.
        broken = VALID.replace(
            "parallel-safe: true",
            "parallel-safe: true\nproviders: claude-only")
        errs = validate(self._write(broken))
        self.assertTrue(
            any("near-miss" in e for e in errs), errs)

    def test_provider_case_variant_near_miss_rejected(self):
        broken = VALID.replace("parallel-safe: true",
                               "parallel-safe: true\nProvider: codex")
        errs = validate(self._write(broken))
        self.assertTrue(any("near-miss" in e for e in errs), errs)

    def test_provider_uppercase_plural_near_miss_rejected(self):
        broken = VALID.replace("parallel-safe: true",
                               "parallel-safe: true\nPROVIDERS: codex")
        errs = validate(self._write(broken))
        self.assertTrue(any("near-miss" in e for e in errs), errs)

    # -- exact valid key must never itself be flagged as a near-miss --

    def test_exact_provider_key_not_flagged_as_near_miss(self):
        ok = VALID.replace("parallel-safe: true",
                           "parallel-safe: true\nprovider: codex")
        errs = validate(self._write(ok))
        self.assertFalse(any("near-miss" in e for e in errs), errs)

    def test_unrelated_key_not_flagged_as_near_miss(self):
        # Guard against over-triggering: an unrelated frontmatter key must
        # never be caught by the near-miss check.
        ok = VALID.replace("parallel-safe: true",
                           "parallel-safe: true\nshard-key: file-list")
        errs = validate(self._write(ok))
        self.assertFalse(any("near-miss" in e for e in errs), errs)
