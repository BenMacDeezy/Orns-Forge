...
class TestDuplicateIdCollisionSurfacesAsSkip(QueueGraphTestCase):
    def test_duplicate_task_id_across_two_files_is_skipped_not_silently_overwritten(self):
        # Two distinct task FILES both declare id: fg-dupe -- before the
        # fix, `tasks[task_id] = {...}` silently overwrote the earlier
        # entry with no increment to graph["skipped"] and no other signal,
        # so one whole task vanished from the graph without a trace. The
        # collision must surface as a skip instead (fg-a10504 convention).
        _write(self.task_dir, "fg-keep", title="Unrelated task", state="ready")
        (pathlib.Path(self.task_dir) / "fg-dupe-first.md").write_text(
            _task("fg-dupe", "First dupe", "ready"), encoding="utf-8"
        )
        (pathlib.Path(self.task_dir) / "fg-dupe-second.md").write_text(
            _task("fg-dupe", "Second dupe", "active"), encoding="utf-8"
        )

        graph = build_graph(self.task_dir)

        # The later-processed file (sorted by path, "second" > "first")
        # must not silently clobber the earlier one with skipped staying 0.
        self.assertEqual(graph["skipped"], 1)
        # Exactly one fg-dupe node survives -- not zero (both discarded),
        # not two (both kept, breaking id-uniqueness).
        dupe_nodes = [n for n in graph["nodes"] if n["id"] == "fg-dupe"]
        self.assertEqual(len(dupe_nodes), 1)

        output = render_mermaid(graph)
        self.assertIn("fg-keep", output)
        self.assertIn("skipped", output.lower())
