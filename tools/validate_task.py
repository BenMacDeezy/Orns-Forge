...
# provider: (fg-c0111/bm-provider-schema-field, docs/specs/2026-07-22-
# phase2-external-workers.md). Closed route-value set = validate_config.py's
# own PROVIDER_ENUM (imported, never restated) union the "claude-only"
# sentinel -- the same union that constant's own PROVIDER_ROLE_KEYS check
# (validate_config.py:448) validates against, kept in sync by reuse rather
# than a second copy.
PROVIDER_VALUES = validate_config.PROVIDER_ENUM | {"claude-only"}
# Fail-closed near-miss key detection (spec's exact rule, not a fuzzy
# heuristic): any key matching /^providers?$/i case-insensitively EXCEPT the
# exact lowercase singular "provider" is a near-miss -- catches the plural
# typo ("providers") and any-case variant ("Provider", "PROVIDERS", ...).
# Under R1's automatic-default routing, a silently-ignored near-miss key
# could leave an intended override unapplied while the task still resolves
# to role-worker's external default -- so this is a validation ERROR, never
# a warning.
PROVIDER_NEAR_MISS_RE = re.compile(r"^providers?$", re.I)
...
def validate(path, warnings=None):
    ...
    # -- provider: (OPTIONAL, bm-provider-schema-field) --
    # Absent means "no task-level override is present" -- not "behaves
    # exactly as before this field existed" (docs/conventions/
    # artifact-formats.md's "Optional task fields -- 2026-07-22" section
    # has the honest absence framing; this validator only shape-checks).
    # A PRESENT key must carry a value in the closed PROVIDER_VALUES set --
    # an empty/blank value is a malformed override, NOT a silent no-op
    # (fail closed: under automatic-default an empty provider: must never
    # slip through as "no override").
    if "provider" in fm:
        provider_val = fm["provider"]
        if not isinstance(provider_val, str) or provider_val not in PROVIDER_VALUES:
            errors.append(
                f"bad provider: {provider_val!r} (want one of: "
                f"{', '.join(sorted(PROVIDER_VALUES))})")

    # Fail-closed near-miss key check: a provider-like misspelled key (the
    # `providers:` plural typo, or any-case variant) must be a hard ERROR,
    # never silently ignored as an unrecognized field.
    for key in fm:
        if key != "provider" and isinstance(key, str) and \
                PROVIDER_NEAR_MISS_RE.match(key):
            errors.append(
                f"bad frontmatter key: {key!r} is a near-miss on the "
                "'provider' field (fail-closed: a provider-like key must "
                "never be silently ignored -- did you mean 'provider'?)")
