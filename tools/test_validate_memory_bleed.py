...
class TestCraftMemoryBleedCheck(unittest.TestCase):
    ...
    # -- BUG 15: the `.forge` comparison in _craft_plugin_root must be
    # case-insensitive --

    def test_case_insensitive_dot_forge_excludes_from_craft_bleed_check(self):
        # A ".Forge" (unexpected case) project directory must still be
        # recognized as the PROJECT store, not misclassified as the craft
        # store (which would wrongly run bleed checks against it).
        root = pathlib.Path(self.tmp.name)
        alt_project_memory = root / "alt-project" / ".Forge" / "memory"
        alt_project_memory.mkdir(parents=True)
        path = alt_project_memory / "proj-fact.md"
        path.write_text(
            FACT_TEMPLATE.format(
                name="proj-fact-altcase",
                body="Ask hockeyben about this."),
            encoding="utf-8")
        warnings = []
        errs = validate(str(path), warnings=warnings)
        self.assertEqual(errs, [])
