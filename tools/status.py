...
def render_board(task_dir, forge_md_path, scope=None, now=None):
    ...
    per skills/queue/references/status-board.md. Never raises."""
    now = now or dt.datetime.now(dt.timezone.utc)
    task_dir = pathlib.Path(task_dir)

    if not task_dir.is_dir():
        return "`.forge/queue/tasks` not found — nothing to show."

    tasks, skipped = load_tasks(task_dir)
    if not tasks:
        return "`.forge/queue/tasks` is empty — nothing to show."
...
def main(argv=None):
    ...
    board_text = render_board(task_dir, forge_md, scope=scope)
