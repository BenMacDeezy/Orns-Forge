...
# Every test here spawns a real bash process. On Windows that spawn is the
# dominant cost, and under a loaded `pytest -n auto` run (which is exactly how
# the gate runs) it can take many seconds before the script's own trivial work
# even starts. A tight budget here produces FALSE failures that look like hook
# bugs -- the same load-flakiness class as the status-board timing test. This
# ceiling is deliberately generous: it exists to catch a genuinely hung script,
# not to measure spawn latency.
HOOK_SUBPROCESS_TIMEOUT = 60
...
TASK_DISPATCH_SCRIPT = REPO_ROOT / "hooks" / "scripts" / "task-dispatch.sh"
...
class TestTaskDispatch(HooksTestBase):
    """task-dispatch.sh merges the two Task|Agent PreToolUse hooks into ONE
    bash process (2026-07-23). The merge is only safe if BOTH checks still
    run on every dispatch -- an `exit` left inside either sourced function
    would kill the dispatcher and silently skip whichever ran second, which
    is exactly the failure these tests exist to catch."""

    def _write_project(self, project_dir, cap_line):
        forge_dir = pathlib.Path(project_dir) / ".forge"
        forge_dir.mkdir(parents=True)
        (forge_dir / "forge.md").write_text(
            "# Forge config\n\n## Budgets\n"
    ...
    def _run_dispatch(self, project_dir, tmpdir, session_id, subtype):
        stdin = json.dumps({"session_id": session_id,
                            "tool_name": "Task",
                            "tool_input": {"subagent_type": subtype,
                                           "prompt": "spawn contract"}})
        extra_env = {"TMPDIR": str(tmpdir).replace("\\", "/")}
        return self._run(TASK_DISPATCH_SCRIPT, project_dir, stdin, extra_env)

    def _log_lines(self, project_dir):
        log = (pathlib.Path(project_dir) / ".forge" / "telemetry"
               / "dispatch-provenance.log")
        if not log.is_file():
            return []
        return [ln for ln in log.read_text(encoding="utf-8").splitlines() if ln]

    def test_registered_as_the_single_task_agent_pretooluse_hook(self):
        hooks_json = json.loads(
            (REPO_ROOT / "hooks" / "hooks.json").read_text(encoding="utf-8")
        )
        entries = [e for e in hooks_json["hooks"]["PreToolUse"]
                   if e.get("matcher") == "Task|Agent"]
        self.assertEqual(len(entries), 1)
        commands = [h["command"] for h in entries[0]["hooks"]]
        self.assertEqual(len(commands), 1, f"expected one spawn, got {commands}")
        self.assertIn("task-dispatch.sh", commands[0])

    def test_both_checks_run_on_one_dispatch(self):
        # The whole point of the merge: one process, both side-effects.
        with tempfile.TemporaryDirectory() as tmp, \
                tempfile.TemporaryDirectory() as counter_tmp:
            self._write_project(tmp, "max-tasks-per-session: 5")
            rc, out, _err = self._run_dispatch(
                tmp, counter_tmp, "sess-dispatch-both", "some-foreign-agent")
            self.assertEqual(rc, 0)
            self.assertEqual(out, "")  # under cap -> budget guard is silent
            lines = self._log_lines(tmp)
            self.assertEqual(len(lines), 1)
            self.assertIn("some-foreign-agent", lines[0])

    def test_provenance_still_logs_when_budget_denies(self):
        # Pre-merge these were two independent hooks, so a deny did not stop
        # the provenance flag from logging. Order must preserve that.
        with tempfile.TemporaryDirectory() as tmp, \
                tempfile.TemporaryDirectory() as counter_tmp:
            self._write_project(tmp, "max-tasks-per-session: 1")
            sid = "sess-dispatch-deny"
            self._run_dispatch(tmp, counter_tmp, sid, "some-foreign-agent")
            rc, out, _err = self._run_dispatch(
                tmp, counter_tmp, sid, "some-foreign-agent")
            self.assertEqual(rc, 0)
            self.assertIn('"permissionDecision":"deny"', out)
            self.assertEqual(len(self._log_lines(tmp)), 2)

    def test_deny_output_is_the_only_stdout(self):
        # The harness parses stdout as a single JSON decision object; the
        # provenance half must contribute nothing to it.
        with tempfile.TemporaryDirectory() as tmp, \
                tempfile.TemporaryDirectory() as counter_tmp:
            self._write_project(tmp, "max-tasks-per-session: 1")
            sid = "sess-dispatch-stdout"
            self._run_dispatch(tmp, counter_tmp, sid, "another-foreign-agent")
            _rc, out, _err = self._run_dispatch(
                tmp, counter_tmp, sid, "another-foreign-agent")
            self.assertEqual(len(out.strip().splitlines()), 1)
            json.loads(out.strip())

    def test_roster_dispatch_logs_nothing_and_is_allowed(self):
        with tempfile.TemporaryDirectory() as tmp, \
                tempfile.TemporaryDirectory() as counter_tmp:
            self._write_project(tmp, "max-tasks-per-session: 5")
            rc, out, _err = self._run_dispatch(
                tmp, counter_tmp, "sess-dispatch-generic", "general-purpose")
            self.assertEqual(rc, 0)
            self.assertEqual(out, "")
            self.assertEqual(self._log_lines(tmp), [])
    ...
    def test_missing_forge_dir_is_silent(self):
        with tempfile.TemporaryDirectory() as tmp, \
                tempfile.TemporaryDirectory() as counter_tmp:
            rc, out, _err = self._run_dispatch(
                tmp, counter_tmp, "sess-dispatch-bare", "some-foreign-agent")
            self.assertEqual(rc, 0)
            self.assertEqual(out, "")

    def test_neither_sourced_function_can_exit_the_dispatcher(self):
        # Static guard: a stray `exit` inside either function body would make
        # the merge silently lossy in a way only some inputs reveal. Both
        # scripts may only `exit` from their standalone-execution guard.
        for script in (BUDGET_GUARD_SCRIPT, AGENT_PROVENANCE_SCRIPT):
            text = script.read_text(encoding="utf-8")
            body = text.split("BASH_SOURCE", 1)[0]
            offenders = [ln for ln in body.splitlines()
                         if re.search(r"(^|[;&|\s])exit\b", ln)
                         and not ln.lstrip().startswith("#")]
            self.assertEqual(
                offenders, [],
                f"{script.name} must `return`, not `exit`, inside its "
                f"sourced function: {offenders}",
            )
