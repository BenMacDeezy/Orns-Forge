...
def build_graph(task_dir, include_done=False):
    ...
    for path in paths:
        ...
        # Two task files declaring the same frontmatter id: `tasks[task_id]
        # = {...}` below would silently overwrite the earlier entry, and
        # one whole task vanishes from the graph with no signal at all.
        # Malformation must surface as a skip, never vanish without a
        # trace (fg-a10504's established convention -- see the
        # blocked-by-inline-list skip case above for the same pattern).
        if task_id in tasks:
            graph["skipped"] += 1
            continue
