...
class TestConsensusRolloutTelemetry(TelemetryTestCase):
    def test_strict_ledger_record_aggregates_all_rollout_metrics(self):
        _write(
            self.task_dir, "fg-consensus-1",
            consensus=(
                "consensus-rollout: c1-rejects=4 c1-resolved=1 c2-rejects=3 "
                "c2-resolved=2 cap-out=yes baseline-cost=1 consensus-cost=3 "
                "baseline-latency-ms=100 consensus-latency-ms=280"
            ),
        )
        _write(
            self.task_dir, "fg-consensus-2",
            consensus=(
                "consensus-rollout: c1-rejects=2 c1-resolved=2 c2-rejects=0 "
                "c2-resolved=0 cap-out=no baseline-cost=1 consensus-cost=2 "
                "baseline-latency-ms=80 consensus-latency-ms=180"
            ),
        )
        metrics = aggregate(self.task_dir)["consensus_rollout"]
        self.assertEqual((metrics["escalations"], metrics["cap_outs"]), (2, 1))
        self.assertEqual(metrics["cap_out_rate"], 0.5)
        self.assertEqual(metrics["judge_yield"]["c1"],
                         {"rejects": 6, "resolved": 3, "rate": 0.5})
        self.assertEqual(metrics["judge_yield"]["c2"],
                         {"rejects": 3, "resolved": 2, "rate": 2 / 3})
        self.assertEqual(metrics["cost_vs_single_pass"],
                         {"baseline": 2, "consensus": 5, "delta": 3, "ratio": 2.5})
        self.assertEqual(metrics["latency_ms_vs_single_pass"],
                         {"baseline": 180, "consensus": 460, "delta": 280,
                          "ratio": 460 / 180})

    def test_missing_or_duplicate_rollout_lines_are_not_invented_as_metrics(self):
        self.assertIsNone(parse_consensus_rollout("cap-out=yes"))
        duplicate = (
            "consensus-rollout: c1-rejects=1 c1-resolved=1 c2-rejects=0 "
            "c2-resolved=0 cap-out=no baseline-cost=1 consensus-cost=1 "
            "baseline-latency-ms=1 consensus-latency-ms=1\n"
        ) * 2
        self.assertIsNone(parse_consensus_rollout(duplicate))
...
class TestPromotedSlugRegistration(TelemetryTestCase):
    """fg-b0307: skills/agent-factory/SKILL.md's promotion "On APPROVE"
    step now includes appending the newly promoted agent's slug to
    tools/telemetry.py's AGENT_SLUGS. This proves the registration
    mechanism itself works end to end -- once a slug is in AGENT_SLUGS,
    a Routing record naming it attributes dispatches correctly -- without
    editing the real AGENT_SLUGS list (patched in via mock.patch.object
    for the duration of the test only)."""

    PROMOTED_SLUG = "forge-widget-fixture"

    def test_hypothetical_promoted_slug_attributes_once_registered(self):
        patched_slugs = sorted(
            telemetry.AGENT_SLUGS + [self.PROMOTED_SLUG], key=len, reverse=True,
        )
        patched_res = [
            (slug, re.compile(r"\b" + re.escape(slug) + r"\b"))
            for slug in patched_slugs
        ]
        _write(
            self.task_dir, "fg-t900",
            routing=(
                f"attempt 1: {self.PROMOTED_SLUG} — sonnet/high — "
                "promoted-slug registration fixture"
            ),
            attempt_log=(
                "attempt 1: dispatched 2026-07-19T00:00:00Z (test fixture)\n\n"
                "attempt 1 verify: sonnet/high verifier -> PASS first attempt. "
                "All criteria held."
            ),
        )
        with mock.patch.object(telemetry, "AGENT_SLUGS", patched_slugs), \
                mock.patch.object(telemetry, "AGENT_SLUG_RES", patched_res):
            report = telemetry.aggregate(self.task_dir)

        self.assertEqual(
            report["agent_dispatch_counts"].get(self.PROMOTED_SLUG), 1
        )
        self.assertEqual(report["first_attempt"], {"pass": 1, "total": 1})

    def test_promoted_slug_absent_from_real_agent_slugs(self):
        # Guards against the fixture accidentally colliding with a real,
        # already-registered slug -- this test proves nothing if it does.
        self.assertNotIn(self.PROMOTED_SLUG, telemetry.AGENT_SLUGS)


class TestFgC0106ProviderJudgeYield(TelemetryTestCase):
    # fg-c0106 (spec-e8a3 Phase 1): a provider-assigned judge's judge-yield
    # line uses a provider-prefixed slug (`codex:<agent-slug>`) -- the SAME
    # line grammar (docs/conventions.md "Verification economics --
    # 2026-07-18 (fg-a10901)"), a provider-qualified slug value, never a new
    # line shape. telemetry.py aggregates it BOTH under the exact slug
    # (backward-compatible with every existing per-slug consumer) AND under
    # a separate per-provider bucket, distinct from per-Claude-agent yield.

    def test_provider_prefixed_slug_parses(self):
        stats = parse_attempt_log(
            "judge-yield: codex:forge-verifier raised=2 survived=1 changed=1"
        )
        self.assertEqual(stats["parsed"], 1)
        self.assertEqual(
            stats["judge_yields"],
    ...
    def test_provider_prefixed_slug_with_severity_suffix_parses(self):
        # fg-a10911's optional p0-p3 suffix composes unchanged with a
        # provider-prefixed slug.
        stats = parse_attempt_log(
            "judge-yield: codex:forge-verifier raised=4 survived=2 changed=1 "
            "p0=1 p1=1 p2=1 p3=1"
        )
        self.assertEqual(stats["parsed"], 1)
        self.assertEqual(stats["judge_yields"][0]["slug"], "codex:forge-verifier")
        self.assertEqual(
            stats["judge_yields"][0]["severity"],
            {"p0": 1, "p1": 1, "p2": 1, "p3": 1},
        )

    def test_malformed_provider_prefix_counts_unparsed(self):
        # Not a valid slug shape (double colon) -- must fall through to
        # unparsed, never a silent partial parse.
        stats = parse_attempt_log(
            "judge-yield: codex::forge-verifier raised=1 survived=1 changed=0"
        )
        self.assertEqual(stats["judge_yields"], [])
        self.assertEqual(stats["unparsed"], 1)

    def test_provider_yield_bucketed_separately_from_exact_slug(self):
        _write(
            self.task_dir, "fg-t301",
            routing="attempt 1: forge-worker — sonnet/high — well-specified",
            attempt_log=(
                "attempt 1: dispatched 2026-07-18T00:00:00Z (ok)\n\n"
                "attempt 1 verify: sonnet/high -> PASS. Green.\n\n"
        ...
        report = aggregate(self.task_dir)
        # Exact-slug bucket (backward-compatible with every existing
        # per-slug consumer) keeps the full provider-qualified slug as its
        # key -- never silently merged into the bare "forge-verifier" key.
        self.assertEqual(
            report["judge_yield"]["codex:forge-verifier"],
            {"raised": 2, "survived": 1, "changed": 1, "verdicts": 1},
        )
        self.assertNotIn("forge-verifier", report["judge_yield"])

    def test_judge_yield_by_provider_separates_codex_from_claude(self):
        _write(
            self.task_dir, "fg-t302",
            routing="attempt 1: forge-worker — sonnet/high — well-specified",
            attempt_log=(
                "attempt 1: dispatched 2026-07-18T00:00:00Z (ok)\n\n"
                "attempt 1 verify: sonnet/high -> PASS. Green.\n\n"
                "judge-yield: forge-verifier raised=3 survived=2 changed=1\n\n"
                "judge-yield: codex:forge-verifier raised=2 survived=1 changed=1\n\n"
                "judge-yield: codex:forge-security raised=1 survived=1 changed=0"
            ),
        )
        report = aggregate(self.task_dir)
        by_provider = report["judge_yield_by_provider"]
        self.assertEqual(
            by_provider["claude"],
            {"raised": 3, "survived": 2, "changed": 1, "verdicts": 1},
        )
        # Two separate codex-prefixed slugs roll up into the SAME "codex"
        # bucket, summed -- distinct from per-Claude-agent yield.
        self.assertEqual(
            by_provider["codex"],
            {"raised": 3, "survived": 2, "changed": 1, "verdicts": 2},
        )

    def test_judge_yield_by_provider_aggregates_across_tasks(self):
        _write(
            self.task_dir, "fg-t303",
            routing="attempt 1: forge-worker — sonnet/high — well-specified",
            attempt_log=(
                "attempt 1: dispatched 2026-07-18T00:00:00Z (ok)\n\n"
                "attempt 1 verify: sonnet/high -> PASS. Green.\n\n"
        ...
        _write(
            self.task_dir, "fg-t304",
            routing="attempt 1: forge-worker — sonnet/high — well-specified",
            attempt_log=(
                "attempt 1: dispatched 2026-07-18T00:00:00Z (ok)\n\n"
                "attempt 1 verify: sonnet/high -> PASS. Green.\n\n"
                "judge-yield: codex:forge-verifier raised=4 survived=1 changed=1"
            ),
        )
        report = aggregate(self.task_dir)
        self.assertEqual(
            report["judge_yield_by_provider"]["codex"],
            {"raised": 6, "survived": 2, "changed": 2, "verdicts": 2},
        )

    def test_no_provider_prefix_yields_no_codex_bucket(self):
        _write(
            self.task_dir, "fg-t305",
            routing="attempt 1: forge-worker — sonnet/high — well-specified",
            attempt_log=(
                "attempt 1: dispatched 2026-07-18T00:00:00Z (ok)\n\n"
                "attempt 1 verify: sonnet/high -> PASS. Green.\n\n"
        ...
        report = aggregate(self.task_dir)
        self.assertIn("claude", report["judge_yield_by_provider"])
        self.assertNotIn("codex", report["judge_yield_by_provider"])

    def test_render_table_shows_provider_breakdown(self):
        _write(
            self.task_dir, "fg-t306",
            routing="attempt 1: forge-worker — sonnet/high — well-specified",
            attempt_log=(
                "attempt 1: dispatched 2026-07-18T00:00:00Z (ok)\n\n"
                "attempt 1 verify: sonnet/high -> PASS. Green.\n\n"
        ...
        report = aggregate(self.task_dir)
        table = render_table(report)
        self.assertIn("Judge yield by provider", table)
        self.assertIn("codex", table)
        self.assertIn("claude", table)
        self.assertIn("3 -> 2 -> 1", table)  # claude bucket
        self.assertIn("2 -> 1 -> 1", table)  # codex bucket
