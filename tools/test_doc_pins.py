...
class TestFg9e0101LatencyPins(unittest.TestCase):
    ...
    def test_kernel_finder_stub_has_pointer_and_enforcement_condition(self):
        ...
        content = (REPO_ROOT / "skills" / "kernel" / "references" / "verify-modes.md").read_text(encoding="utf-8")
...
class TestFg9e0201LowRiskVerifyPins(unittest.TestCase):
    ...
    def test_kernel_has_low_risk_routing_paragraph_with_exact_citation(self):
        ...
        content = (REPO_ROOT / "skills" / "kernel" / "references" / "verify-modes.md").read_text(
            encoding="utf-8"
        )
...
class TestFgA10102RoutingTuningPins(unittest.TestCase):
    ...
    def test_kernel_learn_cites_routing_tuning_section_by_exact_name(self):
        ...
        # unchanged, only the file read.
        content = (REPO_ROOT / "skills" / "kernel" / "references" / "routing-tuning.md").read_text(
            encoding="utf-8"
        )
        self.assertIn(
            '"Routing-tuning recommendations (Evolve analogue) — 2026-07"',
...
class TestFgA10802GruntPins(unittest.TestCase):
    ...
    def test_readme_agent_count_is_twenty_three(self):
        ...
        # this wave, and already carries "20 commands"). Agent count bumped
        ...
        content = self._readme_content()
        self.assertIn("a routed roster of twenty-three agents", content)
        self.assertIn("**45 skills \u00b7 23 agents \u00b7 20 commands**", content)
        self.assertIn("Twenty-three routed agents, each spawned by the kernel", content)
...
class TestFgA10213RoleLabelPins(unittest.TestCase):
    ...
    # a separate axis from TestFg9f0101PersonaPins.CANONICAL_PERSONAS
    ...
    def test_full_persona_role_mapping_present(self):
        ...
        self.assertEqual(
            ...
            set(TestFg9f0101PersonaPins.CANONICAL_PERSONAS.values()),
        )
        section = self._section()
    ...
    def test_kernel_dispatch_line_uses_role_format(self):
        content = re.sub(
            r"\s+", " ", self.KERNEL_PATH.read_text(encoding="utf-8")
        )
        self.assertIn(
            'Any human-visible dispatch label is "<Persona> (<role>)" — no '
    ...
    def test_ship_skill_cites_role_label_for_judge_fanout(self):
        ...
        content = re.sub(
            r"\s+",
            " ",
            (REPO_ROOT / "skills" / "ship" / "SKILL.md").read_text(
                encoding="utf-8"
            ),
        )
        self.assertIn(
...
class TestFgB0302FastPathMintFlowPins(unittest.TestCase):
    ...
    def test_fast_path_heading_exists(self):
        ...
        content = self.AGENT_FACTORY_PATH.read_text(encoding="utf-8")
...
class TestFgB0303EphemeralDispatchMechanicsPins(unittest.TestCase):
    ...
    def test_kernel_verify_mode_3_realigned_no_raw_generic_carveout(self):
        ...
        content = re.sub(r"\s+", " ", ref_path.read_text(encoding="utf-8"))
        section = content.split(
    ...
    def test_kernel_no_remaining_raw_generic_dispatch_language(self):
        ...
        kernel_content = self.KERNEL_PATH.read_text(encoding="utf-8")
...
class TestFgb0305_0306PromotionRetentionPins(unittest.TestCase):
    ...
    def test_conventions_new_section_is_tail_appended(self):
        """The new section must be the LAST '## ' heading in the file —
        tail-append discipline, never inserted mid-document."""
        content = _read_path(self.CONVENTIONS_PATH)
        headings = re.findall(r"^## .+$", content, re.MULTILINE)
        self.assertEqual(headings[-1], self.CONVENTIONS_HEADING)
    ...
    def test_librarian_has_promotion_pass(self):
        content = self.LIBRARIAN_PATH.read_text(encoding="utf-8")
        normalized = re.sub(r"\s+", " ", content)
        self.assertIn("tools/agent_usage.py", normalized)
...
class TestFgB0404WorktreeDisciplineSkillPins(unittest.TestCase):
    ...
    def test_migrator_has_worktree_discipline_attachment(self):
        content = self.MIGRATOR_PATH.read_text(encoding="utf-8")
        section = content.split("## Attached skills")[1].split("##")[0]
        self.assertIn("- worktree-discipline —", section)
