...
AUTO_PARALLEL_CEILING = 16


def resolve_max_parallel_tasks(value, cpu_count=None):
    """Resolve a `max-parallel-tasks` setting to a concurrency window.

    Returns a positive int (the window), or None meaning UNBOUNDED.

    - `"auto"` (the default, and what an absent key means): derive it from
      this machine -- `cpu_count - 2`, floored at 1 and capped at
      `AUTO_PARALLEL_CEILING`. Leaving two cores free keeps the orchestrating
      session responsive while workers build; the ceiling keeps N concurrent
      worktree installs/builds/test-runs from thrashing disk and RAM (past
      the physical ceiling you get slower and flakier runs, not faster, and
      bogus build failures cost real tokens on bounce + re-verify). Mirrors
      the harness's own `min(16, cores - 2)` agent-concurrency shape.
    - `"none"`: UNBOUNDED -- returns None. Explicit human opt-out; whoever
      sets it owns the resource contention.
    - a positive int: used verbatim, no derivation.

    This is the single resolver the kernel's sliding-window dispatch reads,
    so `auto` means the same thing everywhere instead of each caller
    re-deriving it.
    """
    if value is None or value == "auto":
        n = cpu_count if cpu_count is not None else (os.cpu_count() or 1)
        return max(1, min(n - 2, AUTO_PARALLEL_CEILING))
    if value == "none":
        return None
    return int(value)
...
def validate(path, warnings=None):
    ...
    if queue_body is not None:
        ...
        if "max-parallel-tasks" in queue:
            ...
            # "auto" (the default) derives the window from this machine's core
            # count; "none" is an explicit human opt-out to unbounded spawns.
            if v not in ("auto", "none") and (
                    not _INT_RE.match(v) or int(v) <= 0):
                errors.append(
                    f"bad max-parallel-tasks: {v!r} "
                    "(want 'auto', 'none', or a positive integer)")
...
def _role_worker_migration_marker(provider):
    """Path (relative to CWD, matching every other `.forge/`-relative path
    this validator resolves -- see `main()`'s default `.forge/forge.md`)
    of the one-time R1-migration acknowledgment marker for a
    `role-worker: <provider>` assignment.

    Belongs to the same machine-local, gitignored, never-committed marker
    family as the per-provider TOFU trust markers
    (`.forge/.trust-providers/<provider>.local`) and the pilot-clearance
    marker (`.forge/.trust-providers/<provider>.pilot-cleared.local`,
    operator-profiles.md, "Pilot-clearance amendment"), but is its own
    file per role-key/provider pair -- acknowledging the migration notice
    is a distinct human decision from TOFU-trusting a provider CLI or
    clearing its pilot gate, and must not be satisfied by either of those
    markers' mere presence.
    """
    return (pathlib.Path(".forge") / ".trust-providers"
            / f"role-worker.{provider}.migration-ack.local")


def _warn_role_worker_migration(provider, warnings, path):
    """Emit the R1 automatic-default migration notice (docs/specs/2026-07-
    22-phase2-external-workers.md, bm-upgrade-migration-warning) exactly
    once per (role-worker, provider) pair on this machine, for this repo.

    A `role-worker: <provider>` profile assignment predates the R1
    automatic-default ROUTE semantics (docs/specs/2026-07-22-phase2-
    external-workers.md, "R1 (RESOLVED)") whenever it was authored before
    that spec shipped -- this validator has no way to know an assignment's
    authoring date, so it warns on every `role-worker: <provider>`
    profile it validates UNTIL the one-time acknowledgment marker exists,
    at which point the warning is suppressed for good (never re-nagged).
    A no-op when `warnings` is None (caller opted out of advisory output)
    or the marker already exists.
    """
    if warnings is None:
        return
    marker = _role_worker_migration_marker(provider)
    if marker.exists():
        return
    # Always displayed forward-slash (`.as_posix()`), matching every other
    # `.forge/...` path this module names in a message (e.g. the
    # pilot-cleared marker string above), regardless of host OS.
    marker_display = marker.as_posix()
    warnings.append(
        f"{path}: role-worker resolves to {provider!r} -- as of the R1 "
        "automatic-default (docs/specs/2026-07-22-phase2-external-"
        f"workers.md), this now routes eligible non-security tasks to "
        f"{provider!r} by DEFAULT, with no per-task `provider:` field "
        "required. If this role-worker assignment was authored before "
        "that spec shipped, review it before relying on automatic-"
        "default routing. This is a ONE-TIME notice: acknowledge it by "
        f"creating {marker_display} (machine-local, gitignored, never "
        "committed -- same family as the per-provider trust markers); "
        "once created, this warning is suppressed for this role-worker/"
        "provider pair.")
...
def _validate_providers_domain(items, errors, warnings, path):
    ...
    (fg-c0101)"), the pilot-gate hard-stop, the security rejection above,
    and (bm-upgrade-migration-warning, docs/specs/2026-07-22-phase2-
    external-workers.md) the R1 automatic-default migration notice for a
    `role-worker: <provider>` assignment. Unknown/renamed keys warn, never
    fail (forward-compat); a removed-capability key degrades to its stock
    default with exactly one stated warning line. `errors` entries are
    unprefixed (the caller wraps them with `path` on return); `warnings`
    entries must include `path` themselves, matching every other warning
    `validate_profile` emits."""
    for key, value in items.items():
        ...
        if key == "enabled-providers":
            ...
            elif key == "role-worker" and value in PROVIDER_ENUM:
                _warn_role_worker_migration(value, warnings, path)
