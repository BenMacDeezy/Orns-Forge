...
class TestSpecValidator(unittest.TestCase):
    ...
    def test_bad_id_fails(self):
        # "zz" fails both the legacy spec-xxxx hex shape AND the new
        # readable-name-id shape (too short, min 3 chars) -- unlike a value
        # such as "spec-1", which the spec-f0c2 name-id convention (see
        # TestReadableNameIds, below) makes legitimately valid.
        errs = validate(self._write(DRAFT.replace("id: spec-a3f1", "id: zz")))
    ...
    def test_three_char_id_now_valid_as_name_id(self):
        ...
        # 4-8 chars after "spec-"). "spec-a3f" is 8 chars total, all
        # lowercase alnum/hyphen, starts/ends alnum, no "--" -- it now
        # legitimately satisfies the readable-name-id shape (see
        # TestReadableNameIds), so it is accepted, not because it's read as
        # hex, but because it happens to also be valid kebab-case.
...
class TestReadableNameIds(unittest.TestCase):
    """spec-f0c2 Amendments — 2026-07-20, item 2: new spec ids may be a
    human-readable kebab-case name (3-40 chars, [a-z0-9][a-z0-9-]*[a-z0-9],
    no leading/trailing '-', no '--' run), in addition to the legacy
    spec-xxxx hex id, which stays valid and grandfathered forever."""
    ...
    def test_readable_name_id_passes(self):
        errs = validate(self._write(
            DRAFT.replace("id: spec-a3f1", "id: color-refactor")))
        self.assertEqual(errs, [])

    def test_short_name_id_passes(self):
        errs = validate(self._write(DRAFT.replace("id: spec-a3f1", "id: abc")))
        self.assertEqual(errs, [])

    def test_max_length_name_id_passes(self):
        name = "a" * 40
        errs = validate(self._write(DRAFT.replace("id: spec-a3f1", f"id: {name}")))
        self.assertEqual(errs, [])
    ...
    def test_too_short_name_id_fails(self):
        errs = validate(self._write(DRAFT.replace("id: spec-a3f1", "id: ab")))
        self.assertTrue(any("id" in e for e in errs))
    ...
    def test_double_hyphen_name_id_fails(self):
        errs = validate(self._write(DRAFT.replace("id: spec-a3f1", "id: ab--cd")))
        self.assertTrue(any("id" in e for e in errs))

    def test_legacy_hex_ids_still_valid(self):
        for legacy in ("spec-a3f1", "spec-4d2a", "spec-e8a3"):
            with self.subTest(legacy=legacy):
                errs = validate(self._write(
                    DRAFT.replace("id: spec-a3f1", f"id: {legacy}")))
                self.assertEqual(errs, [])
