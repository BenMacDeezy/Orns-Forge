...
class TestFgB0205UninstallPins(unittest.TestCase):
    ...
    below is unique to text this task added -- never a phrase update.md,
    banner.md, or customization-persistence.md already owns in text this
    task cites but does not restate."""
    ...
    def test_frontmatter_description_present(self):
        ...
        self.assertIn(
            ...
            "clean up shims, offer .forge/ removal, print an itemized report",
            content,
        )
    ...
    def test_banner_shim_removal_delegates_to_banner_install_tool(self):
        content = self._uninstall()
        self.assertIn(
            "Run\n   `python tools/banner_install.py uninstall` (the same "
            "tool\n   `/forge:banner uninstall` uses, per "
            "`commands/banner.md`)",
            content,
        )
        self.assertIn(
            "This is the only shim-cleanup Forge performs — no other\n"
            "   user-space file is touched (`docs/customization-"
            "persistence.md`, \"Banner\n   launcher shim (örn banner)\" "
            "row).",
            content,
        )
    ...
    def test_forge_dir_offer_via_one_structured_askuserquestion(self):
        content = self._uninstall()
        self.assertIn(
            "4. **Offer `.forge/` removal via one structured confirm.** "
    ...
    def test_itemized_removed_list_and_never_overclaim_sentence(self):
        content = self._uninstall()
        self.assertIn(
            "5. **Print an exact itemized removed-list.** One line per "
            "thing actually\n   removed this run, e.g.:",
            content,
        )
        self.assertIn(
            "Never list something as removed that wasn't — a skipped "
            "marketplace\n   entry, a declined `.forge/` removal, or a "
            "shim surface `banner_install.py\n   uninstall` reported as "
            "already-absent are reported as \"not removed\" /\n   \"kept\", "
    ...
    def test_never_deletes_forge_dir_without_step4_confirmation(self):
        content = self._uninstall()
        self.assertIn(
            "- Never deletes `.forge/` without the explicit structured "
            "confirmation in\n  step 4 — declining always leaves it fully "
            "intact.",
            content,
        )
    ...
    def test_never_modifies_beyond_confirmed_project_or_unowned_user_space(self):
        ...
        self.assertIn(
            ...
            "under user space that\n  Forge did not itself install "
            "(`docs/customization-persistence.md` is the\n  source of "
            "truth for which user-space surfaces Forge owns — the banner\n"
            "  shim only).",
            content,
        )
