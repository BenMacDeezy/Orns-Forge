...
class TestFgB0203PortCommandPins(unittest.TestCase):
    ...
    def test_map_architecture_names_port_entry_point_and_count(self):
        ...
        # directed, 2026-07-22) then bumped 26->25 back down. This pin now
        # asserts the current count, not the count port's own addition
        # produced.
        self.assertIn("twenty-five slash commands", content)
        self.assertIn(
            "Twenty-five thin slash-command entry points",
            content,
        )
    ...
    def test_docs_architecture_names_twenty_four_commands(self):
        ...
        self.assertIn(
            "Twenty-five thin slash-command entry points under "
            "`commands/*.md`",
            content,
        )
