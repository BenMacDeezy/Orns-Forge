...
class TestFgA10101TelemetryPins(unittest.TestCase):
    ...
    def test_readme_lists_telemetry_command(self):
        # "25 commands" (not 19): inventory count from commands/*.md.
        ...
        # the count dropped back to 25 with the startup-banner removal
        # (owner-directed, 2026-07-22) -- this hardcoded companion pin
        # stays in sync with README's current count.
        readme = _cached_read_text((REPO_ROOT / "README.md"))
        self.assertIn("/forge:telemetry", readme)
        self.assertIn("25 commands", readme)
    ...
    def test_map_command_count_is_twenty(self):
        ...
        wave/parallelization blueprint command). Bumped back 26->25 by the
        startup-banner removal (owner-directed, 2026-07-22: commands/banner.md
        deleted, the /forge:banner entry point removed)."""
        ...
        self.assertIn("Twenty-five thin slash-command entry points", content)
