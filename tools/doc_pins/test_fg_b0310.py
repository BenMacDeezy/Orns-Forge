...
class TestFgB0310DispatchProvenanceFlagPins(unittest.TestCase):
    ...
    def test_hook_script_exists_and_registered(self):
        # Registration is INDIRECT since 2026-07-23: the two Task|Agent
        # PreToolUse hooks were merged behind hooks/scripts/task-dispatch.sh,
        # which sources this script and calls its function, so an agent
        # dispatch costs one bash spawn instead of two. The invariant this
        # pin protects is unchanged -- every Task|Agent dispatch still runs
        # the provenance flag -- so assert the whole chain, not just the
        # script's name appearing in hooks.json.
        script = REPO_ROOT / "hooks" / "scripts" / "agent-provenance-flag.sh"
        self.assertTrue(script.is_file())
        hooks_json = _cached_read_text((REPO_ROOT / "hooks" / "hooks.json"))
        self.assertIn("task-dispatch.sh", hooks_json)
        self.assertIn('"matcher": "Task|Agent"', hooks_json)
        dispatch = _cached_read_text(
            REPO_ROOT / "hooks" / "scripts" / "task-dispatch.sh"
        )
        self.assertIn("agent-provenance-flag.sh", dispatch)
        self.assertIn("forge_agent_provenance", dispatch)
