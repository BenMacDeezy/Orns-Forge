...
class TestFgC0111ProviderWorkerPins(unittest.TestCase):
    ...
    def test_worktree_hard_rule_4_identical_sentence_present(self):
        # 2026-07-24: §7.2 generalized to worktree-OR-in-place (degraded
        # mode). Hard Rule 4 now holds "in BOTH cases" and says "worker's
        # tree" instead of "worktree", since an in-place worker has none.
        # The invariant this pins is unchanged: the external worker never
        # touches .forge/; all .forge/ writes are kernel-only.
        content = self._provider_judges()
        self.assertIn(
            "Hard Rule 4 holds identically in BOTH cases: the external "
            "worker never\ntouches `.forge/`; every `.forge/` write is "
            "kernel-only, on the main\nbranch, never inside the worker's "
            "tree.",
            content,
        )

    def test_provider_worker_runs_in_place_when_worktrees_unavailable(self):
        # The 2026-07-24 fix: a codex mutating worker is NOT excluded just
        # because worktrees are broken -- it mirrors the Claude worker's
        # mode, which is in-place on the sequential/degraded path.
        content = self._provider_judges()
        self.assertIn("Sequential / degraded dispatch", content)
        self.assertIn("runs IN-PLACE too", content)
        self.assertIn(
            "NEVER excluded merely because worktrees are\n  unavailable",
            content,
        )
        self.assertIn("degraded-worktree-mode.md", content)
